...
test.describe('tauri build configuration', () => {
  ...
  // The app OAuth return uses a custom scheme, and `tauri-plugin-deep-link`'s
  // build script is the single source for both platform declarations: it derives
  // the Android intent-filter and the iOS `CFBundleURLTypes` entry from
  // `tauri.conf.json`. Nothing here may be hand-written into `Info.ios.plist` or
  // `src-tauri/gen/`, which is why the plist must stay free of `CFBundleURLTypes`.
  test("the app OAuth deep link matches its shared callback URL", async () => {
    const pathsSource = await readFile(
      path.resolve(__dirname, "../packages/contracts/src/paths.ts"),
      "utf8",
    );
    const callback = pathsSource.match(
      /export const APP_OAUTH_CALLBACK_URL = "([^"]+)" as const/,
    )?.[1];
    assert.ok(callback, "APP_OAUTH_CALLBACK_URL is no longer a shared constant");
    const expected = new URL(callback);

    const tauriConfig = JSON.parse(
      await readFile(`${webRoot}/src-tauri/tauri.conf.json`, "utf8"),
    );
    const deepLink = tauriConfig.plugins?.["deep-link"];
    assert.ok(deepLink, "plugins.deep-link is missing from tauri.conf.json");
    assert.equal(deepLink.desktop, undefined);
    assert.ok(Array.isArray(deepLink.mobile));
    assert.equal(deepLink.mobile.length, 1);
    const mobile = deepLink.mobile[0];
    assert.deepEqual(mobile.scheme, [expected.protocol.replace(":", "")]);
    assert.equal(mobile.host, expected.host);
    assert.deepEqual(mobile.pathPrefix, [expected.pathname]);

    const webPackage = JSON.parse(
      await readFile(`${webRoot}/package.json`, "utf8"),
    );
    assert.ok(webPackage.dependencies["@tauri-apps/plugin-deep-link"]);

    const cargoManifest = await readFile(
      `${webRoot}/src-tauri/Cargo.toml`,
      "utf8",
    );
    assert.match(cargoManifest, /tauri-plugin-deep-link = "2"/);
    const cargoLock = await readFile(`${webRoot}/src-tauri/Cargo.lock`, "utf8");
    assert.match(
      cargoLock,
      /\[\[package\]\]\s+name = "tauri-plugin-deep-link"\s+version = "2\./,
    );

    const rustEntry = await readFile(`${webRoot}/src-tauri/src/lib.rs`, "utf8");
    assert.match(rustEntry, /\.plugin\(tauri_plugin_deep_link::init\(\)\)/);
    ...
    const iosPlist = await readFile(`${webRoot}/src-tauri/Info.ios.plist`, "utf8");
    assert.ok(
      !iosPlist.includes("CFBundleURLTypes"),
      "CFBundleURLTypes must be generated by the plugin, not hand-written",
    );
  });
});
