...
- [ ] AC1 iOS / Android / Windows / Linux / macOS 各能解析出对应系统标记，未知输入降级为未知态
- [ ] AC2 手机与 PC 能区分；平板等模糊形态有明确定义的归属，不出现空白或串味
- [ ] AC3 同型号但 UA 不同的设备可区分；UA 完全相同的设备靠 `id` 前 4 位后缀区分，后缀在会话刷新后保持稳定。测试 fixture 必须换掉现有的 `session-current` / `session-other`——两者 `slice(0,4)` 都是 `sess`，会掩盖后缀区分能力，本条无法被证明
- [ ] AC3a 「已知系统但端类型未知」与「完全未知设备」渲染为不同文案，不存在两者撞成同一行的输入
- [ ] AC4 图标随端类型变化（手机 / 平板 / PC / 未知），未知态有对应图标，且错误判定不会让图标与文案互相矛盾
- [ ] AC5 原始 UA 仍可在界面上取到，不因改版丢失
- [ ] AC6 纯函数单测覆盖真实 UA 样本清单与边界输入（空串、null、超长、畸形）
- [ ] AC7 `account-security-page.test.tsx` 中依赖 UA 原文的断言同步更新（6 处断言 `:297/:309/:312/:323/:332/:425`，外加 2 处 fixture 字面量 `:49/:59`），改后测试通过。`:300` 附近是「当前设备」徽标断言，与 UA 无关，不要误改
- [ ] AC8 zh-CN 与 en 均为新增文案补齐键，无缺键回退
- [ ] AC9 `platformSessionDeviceSchema` 无字段增删或语义变更；无 API 层改动
