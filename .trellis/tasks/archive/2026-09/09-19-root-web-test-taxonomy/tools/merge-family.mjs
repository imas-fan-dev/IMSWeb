...
const renameIdentifier = (text, from, to) => {
    if (!IDENTIFIER.test(from)) throw new Error(`拒绝改名：'${from}' 不是合法标识符（import 解析异常）`);
...
const resolveSpecifier = (fromDir, specifier) => {
    // The API tests reach production through `@/` -> tests/../src and the Web
    // tests through `~/` -> tests/unit/../../app; try both bases so one tool
    // serves either tree.
    const bases = [];
    if (specifier.startsWith("@/") || specifier.startsWith("~/")) {
        for (const base of ["src", "app", "../app"]) {
            bases.push(join(root, "..", base, specifier.slice(2)));
        }
    } else if (specifier.startsWith(".")) {
        bases.push(join(fromDir, specifier));
    }
...
const analyse = (entry) => {
    ...
    for (const local of [...binders.keys()].sort()) {
        ...
        // Every binding name here came out of parseImport; a name that is not a
        // plain identifier means the statement was not an import we understand.
        // Refuse the family instead of corrupting it.
        if (!IDENTIFIER.test(local)) {
            reasons.push(`import 解析异常：把 '${local}' 当成了标识符（来自 ${ownerModules.join(", ")}）`);
            continue;
        }
