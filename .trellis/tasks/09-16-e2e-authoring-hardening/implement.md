...
- 期望结果：App 三个 project 全部通过，单次耗时明显低于 20s（实测 6.0–7.7s）；Web 目标用例在 chromium-desktop 与 chromium-mobile 通过（实测 desktop 7.5/9.5s、mobile 12.8s）。
...
单次提交即可回滚：还原两个 spec 的调用点、删除 `fixtures/toast.ts`、回退两份文档。
