...
| `packages/contracts/` | `@imsweb/contracts` | API↔Web 共享线格式契约与路径 builders |
...
该命令会检查本机环境，启动 PostgreSQL、Valkey 和 RustFS，等待依赖就绪，幂等应用数据库迁移，
再启动支持热更新的 Hono API 与 React Web。默认地址如下：
...
| `pnpm run dev:down`         | 停止本地 PostgreSQL、Valkey 与 RustFS，保留数据卷 |
...
或 `pnpm --filter @imsweb/contracts run <command>`，不再为同一个动作增加多层转发别名。
...
[资产来源记录](docs/governance/assets.md)。没有明确再分发授权的图片、字体、
...
仓库级文档的分类、规范和权威来源见[文档中心](docs/README.md)。

- [贡献指南](CONTRIBUTING.md)
- [架构文档](docs/architecture/)
- [开发文档](docs/development/)
- [运维文档](docs/operations/)
- [迁移文档](docs/migrations/)
