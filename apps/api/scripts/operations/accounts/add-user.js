...
const configuredDbPath = process.env.IMS_DB_PATH;
...
if (!configuredDbPath) {
    console.error('Set IMS_DB_PATH explicitly to the authoritative core database.');
    process.exit(1);
}

const dbPath = path.resolve(projectRoot, configuredDbPath);
if (!fs.existsSync(dbPath) || !fs.statSync(dbPath).isFile()) {
    console.error(`IMS_DB_PATH is not an existing database file: ${dbPath}`);
    process.exit(1);
}
