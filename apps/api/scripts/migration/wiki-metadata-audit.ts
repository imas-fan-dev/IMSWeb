...
import { createNodeServices } from "../../src/runtime/node-services";
import {
    agencyIconObjectKey,
    idolMediaObjectKey,
    storyObjectKey,
} from "../../src/domains/content/wiki/service";
import type { ObjectStorage } from "../../src/ports/object-storage";
import type { BackofficeAuthRepository } from "../../src/ports/repositories/admin";
import type { StoryRepository } from "../../src/ports/repositories/wiki";

interface WikiMetadataServices {
...
export function parseWikiMetadataAuditArguments(argv: string[]): Options {
    const options: Options = {
        ...
        report: path.join(
            projectRoot,
            "data/migration/wiki-metadata-audit.json",
        ),
        strict: false,
    };
...
async function semanticIdolMediaKey(
    services: Pick<WikiMetadataServices, "storage">,
    agencyCode: string,
    folderName: string,
): Promise<string | null> {
...
export async function applyExistingSemanticMedia(
    services: Pick<WikiMetadataServices, "story" | "storage">,
...
async function buildReport(
    ...
    for (const member of members) {
        membersByIdol.set(
            member.idol_id,
            (membersByIdol.get(member.idol_id) ?? 0) + 1,
        );
    }
    ...
    const duplicateMembers = [...membersByIdol]
        ...
        .map(([idolId]) => idolId);
    const fallbackCounts = agencies
        .map((agency) => ({
            agencyId: agency.id,
            count: groups.filter(
                (group) => group.agency_id === agency.id && group.is_fallback,
            ).length,
        }))
        .filter((item) => item.count !== 1);
    const duplicateOrders = {
        agencies: duplicateValues(
            agencies
                .filter((agency) => agency.wiki_enabled)
                .map((agency) => agency.display_order),
        ),
        groups: agencies.flatMap((agency) =>
            duplicateValues(
                groups
    ...
    const invalidColors = [
        ...agencies
            .filter((agency) => !validColor(agency.color))
            .map((agency) => `agency:${agency.id}`),
        ...groups
            .filter((group) => !validColor(group.color))
            .map((group) => `group:${group.id}`),
        ...idols
            .filter((idol) => idol.color && !validColor(idol.color))
            .map((idol) => `idol:${idol.id}`),
    ];
    ...
    const unassociatedIdolMedia = semanticIdolMedia
        .filter(
            (item) => item.semanticKey && item.currentKey !== item.semanticKey,
        )
        .map((item) => ({
            idolId: item.idolId,
    ...
    const categoryAudits = await mapLimit(enabledIdols, 8, async (idol) => {
        ...
        return {
            ...
            storyKeys: stories.flatMap((story) =>
                story.image_file
                    ? [
                          storyObjectKey(
                              idol.agency_code,
    ...
    const invalidObjectKeys = [
        ...new Set(mediaKeys.filter((key) => !validObjectKey(key))),
    ];
    const missingObjects = (
        await mapLimit(
            ...
            async (key) => ({
                key,
                exists: await services.storage.exists(key),
            }),
        )
    )
    ...
    const issueCount =
        unassignedIdols.length +
        duplicateMembers.length +
        fallbackCounts.length +
        duplicateOrders.agencies.length +
        duplicateOrders.groups.length +
        duplicateOrders.members.length +
        unknownCategories.length +
        invalidColors.length +
        invalidObjectKeys.length +
        unassociatedIdolMedia.length +
...
async function closeServices(services: WikiMetadataServices) {
    await Promise.allSettled(
        [
            services.storage.close?.(),
            (services.story as { close?: () => Promise<void> }).close?.(),
            (
                services.backofficeAuth as { close?: () => Promise<void> }
            ).close?.(),
...
async function main() {
    ...
    // SAFETY: createNodeServices always composes storage, story, and Backoffice repositories.
    const services =
        (await createNodeServices()) as unknown as WikiMetadataServices;
    try {
        const applied = options.apply
            ? await applyExistingSemanticMedia(services)
            : [];
