...
function mediaObjectKey(key) {
    if (key === 'uploads/information/index.json') return 'editorial/information/index.json';
    ...
    if (['uploads/event/original', 'uploads/event/thumb', 'uploads/events'].includes(prefix)) {
        return `editorial/events/assets/${file.stem}/poster.${file.extension}`;
    }
    if (['uploads/information', 'uploads/information/original'].includes(prefix)) {
...
function semanticObjectKey(key) {
    ...
    if (segments[0] === 'Wiki' && segments[1] === 'static' && segments.length >= 4) {
...
function parseArguments(argv, environment = process.env) {
    ...
    if (options.deleteSource && !options.apply) throw new Error('--delete-source requires --apply');
    if (options.apply && (!options.bucket || options.confirmBucket !== options.bucket)) {
        throw new Error(`Apply requires --confirm-bucket ${options.bucket || '<IMS_S3_BUCKET>'}`);
    }
    if (!options.manifest) {
...
function indexedSourcePhysicalKey(config, snapshot) {
    return snapshot.physicalKey || physicalKey(
        config,
        `__ims_s3/objects/${snapshot.objectId}`
    );
}
...
async function rawGet(client, config, logicalKey) {
    return rawGetPhysical(client, config, physicalKey(config, logicalKey), logicalKey);
}
...
async function inventory(storage, state, client, config) {
    ...
    for (const prefix of LEGACY_SCOPES) {
        for (const object of await storage.list(prefix)) {
            ...
            if (!snapshot || !['pending', 'ready'].includes(snapshot.state)) {
                throw new Error(`Managed migration source has no readable state: ${object.key}`);
            }
            const destination = semanticObjectKey(object.key);
            const sourcePhysicalKey = indexedSourcePhysicalKey(config, snapshot);
            if (
                object.key === destination &&
                sourcePhysicalKey === semanticPhysicalKey(config, object.key, snapshot.objectId)
            ) {
                continue;
            }
    ...
    for (const prefix of LEGACY_SCOPES.filter((scope) => scope !== 'site-packages/')) {
...
async function migrateEntry(storage, state, client, config, entry, options) {
    const source = entry.sourceKind === 'indexed'
        ? await rawGetPhysical(client, config, entry.sourcePhysicalKey, entry.source)
    ...
    let destination = entry.source === entry.destination ? null : await storage.get(entry.destination);
    let rewritten = false;
    if (!destination || digest(destination.body) !== sourceSha256 ||
    ...
    if (!verified || verified.size !== source.size || digest(verified.body) !== sourceSha256) {
        throw new Error(`Verification failed: ${entry.destination}`);
    }
    if (options.deleteSource) {
        if (entry.sourceKind === 'indexed') {
            if (entry.source !== entry.destination) await storage.delete(entry.source);
        ...
        const remaining = entry.sourceKind === 'indexed'
            ? Boolean(await rawGetPhysical(
                client,
                config,
...
async function main() {
    ...
    if (config.type !== 's3') throw new Error('Semantic object migration requires S3 storage');
    ...
    try {
        ...
        const entries = await inventory(services.storage, state, client, config);
        const startedAt = new Date().toISOString();
        let migrated = [];
        if (options.apply) {
            migrated = await parallelMap(entries, options.concurrency, async (entry, index) => {
                ...
                if ((index + 1) % 100 === 0 || index + 1 === entries.length) {
                    process.stderr.write(`Migrated ${index + 1}/${entries.length}\n`);
                }
                return result;
            });
