...
function normalizeOrigin(value) {
    const source = value || DEFAULT_SOURCE_ORIGIN;
    let url;
    try {
        url = new URL(source);
    } catch (error) {
        throw new Error(`Source origin is not a valid URL: ${source}`, {
            cause: error,
        });
    }
    if (url.username || url.password || url.search || url.hash) {
        throw new Error(
            "Source origin must not contain credentials, query, or fragment",
        );
    }
...
function parseArguments(argv) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        else if (argument === "--page-concurrency")
            options.pageConcurrency = next();
        else if (argument === "--asset-concurrency")
            options.assetConcurrency = next();
    ...
    options.manifest = path.resolve(
        options.manifest || path.join(options.stagingDir, "manifest.json"),
    );
...
async function createPostgresStoryRepository(
    ...
    const {
        ...
        parseNodeDatabaseConfig: require("../../src/config/database.ts")
            .parseNodeDatabaseConfig,
        PostgresConnection:
            require("../../src/infra/db/postgresql/connection.ts")
                .PostgresConnection,
        PostgresqlSchemaStrategy:
            require("../../src/infra/db/postgresql/schema-strategy.ts")
                .PostgresqlSchemaStrategy,
        SqlStoryRepository:
            require("../../src/infra/db/repositories/story-repository.ts")
                .SqlStoryRepository,
    };
...
function buildIdolIndex(rows) {
    ...
    for (const row of rows) {
        ...
        if (
            ...
            throw new Error(
                "Story database contains an incomplete agency/idol mapping",
            );
        }
        const key = `${row.agency_name.normalize("NFC")}\0${row.idol_name.normalize("NFC")}`;
        if (index.has(key))
            throw new Error(
                `Duplicate agency/idol mapping: ${key.replace("\0", "/")}`,
            );
        index.set(key, {
...
function safeSegment(value) {
    let segment;
    try {
        segment = decodeURIComponent(value).normalize("NFC");
    } catch (error) {
        throw new Error(
            `Wiki URL path segment has invalid percent-encoding: ${value}`,
            {
                cause: error,
            },
        );
...
function decodedSegments(url) {
    ...
    try {
        ...
        throw new Error(`Wiki asset URL is invalid: ${url}`, { cause: error });
    }
    return parsed.pathname.split("/").filter(Boolean).map(safeSegment);
}
...
function safeObjectKey(segments) {
    ...
    if (Buffer.byteLength(key, "utf8") > 1024)
        throw new Error(`Object key is too long: ${key}`);
    return key;
}
...
function mapAssetUrl(assetUrl, idolIndex) {
    ...
    if (["icon", "css", "assets"].includes(segments[0])) {
        const agencyIcon =
            ...
            segments[1] === "agencies" &&
            segments.length === 3 &&
            /^[a-z0-9_-]+\.webp$/i.test(segments[2]);
        const agencyCode = agencyIcon
            ? segments[2].slice(0, -".webp".length)
            : null;
        return {
            kind: "wiki-static",
...
function canonicalStoryUrl(value, baseUrl, sourceOrigin) {
    ...
    if (resolved.origin !== sourceOrigin || resolved.pathname !== "/story")
        return null;
...
function extractCssReferences(css, baseUrl, sourceOrigin) {
    ...
    for (const pattern of patterns) {
        for (const match of css.matchAll(pattern)) {
            const candidate = canonicalAssetUrl(
                match[2] || match[1],
                baseUrl,
                sourceOrigin,
            );
        ...
        if (character === '"') string = true;
            ...
            if (depth === 0) {
                try {
                    ...
                    throw new Error(`${assignment} contains invalid JSON`, {
                        cause: error,
                    });
                }
            }
    ...
    function visit(node) {
        for (const attribute of node.attrs || []) {
            if (["src", "href", "poster"].includes(attribute.name)) {
                ...
                for (const asset of extractCssReferences(
                    attribute.value,
                    baseUrl,
                    sourceOrigin,
                )) {
...
function detectedContentType(body) {
    ...
    if (body[0] === 0xff && body[1] === 0xd8 && body[2] === 0xff)
        return "image/jpeg";
    if (body.subarray(0, 6).toString("ascii").startsWith("GIF8"))
        return "image/gif";
    if (
    ...
    const prefix = body
        .subarray(0, Math.min(body.length, 512))
        .toString("utf8")
        .trimStart();
    if (/^<svg[\s>]/i.test(prefix) || /^<\?xml[\s\S]*?<svg[\s>]/i.test(prefix))
...
function resolvedContentType(sourceUrl, header, body) {
    ...
    try {
        ...
        throw new Error(`Asset source URL is invalid: ${sourceUrl}`, {
            cause: error,
        });
    }
    ...
    if (
        ...
        if (!detected?.startsWith("image/"))
            throw new Error(`Asset content is not an image: ${sourceUrl}`);
        return detected;
    }
    return (
        detected ||
...
async function fetchBody(
    url,
    { maxBytes, accept, sourceOrigin, attempts = 3 } = {},
) {
    let lastError;
    for (let attempt = 1; attempt <= attempts; attempt += 1) {
        try {
            ...
            if (new URL(response.url).origin !== sourceOrigin) {
                throw new Error(
                    `Cross-origin redirect rejected: ${url} -> ${response.url}`,
                );
            }
            if (!response.ok)
            ...
            if (declared && declared > maxBytes)
                throw new Error(`Response exceeds byte limit: ${url}`);
...
async function uploadAsset(storage, stagingDir, asset) {
    ...
    if (
        ...
        sha256(verified.body) !== asset.sha256
    ) {
        throw new Error(
            `Uploaded object verification failed: ${asset.objectKey}`,
        );
    }
    return {
        status: current ? "replaced" : "uploaded",
        etag: result.etag || verified.etag || null,
    };
}
...
async function uploadExistingManifest(options, idolIndex, providedStorage) {
    const source = await fsp.readFile(options.manifest, "utf8");
    let parsed;
    try {
        parsed = JSON.parse(source);
    } catch (error) {
        throw new Error(
            `Existing Wiki manifest is not valid JSON: ${options.manifest}`,
            {
                cause: error,
            },
        );
    ...
    if (
        ...
        throw new Error(
            "Existing Wiki manifest must be a complete version 1 document",
        );
    }
    if (normalizeOrigin(parsed.sourceOrigin) !== options.sourceOrigin) {
        throw new Error(
            "Existing Wiki manifest source origin does not match --source-origin",
        );
    }
    if (parsed.errors?.length)
        throw new Error("Existing Wiki manifest contains errors");
    ...
    await mapConcurrent(
        ...
        async (asset) => {
            if (
                ...
                throw new Error(
                    "Existing Wiki manifest contains an invalid asset entry",
                );
            if (objectKeys.has(asset.objectKey))
                throw new Error(
                    `Duplicate manifest object key: ${asset.objectKey}`,
                );
            objectKeys.add(asset.objectKey);
            const remapped = mapAssetUrl(asset.sourceUrl, idolIndex);
            if (remapped.objectKey !== asset.objectKey) {
                throw new Error(
                    `Manifest business mapping changed: ${asset.objectKey}`,
                );
            }
            const stagedFile = path.resolve(
                options.stagingDir,
                asset.stagedPath,
            );
            ...
            if (
                ...
                throw new Error(
                    `Unsafe manifest staging path: ${asset.stagedPath}`,
                );
            }
            ...
            if (
                ...
                throw new Error(
                    `Staged Wiki asset failed integrity verification: ${asset.objectKey}`,
                );
            }
        },
    );
    ...
    parsed.summary = {
        ...
        uploaded: results.filter(
            (result) =>
                result.status === "uploaded" || result.status === "replaced",
        ).length,
...
async function syncWikiMedia(options, idolIndex, providedStorage) {
    ...
    const indexReferences = extractHtmlReferences(
        indexHtml,
        indexUrl,
        options.sourceOrigin,
        parse,
    );
    const indexPath = await writeStagedFile(
        options.stagingDir,
    ...
    const remoteStories = [...indexReferences.stories.values()].sort(
        (left, right) => compareUtf8(left.url, right.url),
    );
    const remoteIdentities = new Set(
        remoteStories.map((story) => `${story.agency}\0${story.idol}`),
    );
    ...
    for (const identity of remoteIdentities) {
        if (!databaseIdentities.has(identity)) {
            manifest.errors.push({
                type: "unknown-remote-story",
                identity: identity.replace("\0", "/"),
            });
        }
    }
    ...
    if (manifest.errors.length) {
        ...
        throw new Error(
            `Remote/local Wiki story inventory differs (${manifest.errors.length} mismatch(es))`,
        );
    }
    ...
    for (const sourceUrl of indexReferences.assets)
        addAsset(sourceUrl, "page:wiki/index");

    const storyResults = await mapConcurrent(
        remoteStories,
        options.pageConcurrency,
        async (story) => {
            ...
            try {
                ...
                for (const sourceUrl of references.assets)
                    addAsset(sourceUrl, `page:story/${pageId}`);
    ...
    while (true) {
        const pending = [...contextsByAsset.keys()].filter(
            (sourceUrl) => !assetsByUrl.has(sourceUrl),
        );
            ...
            async (sourceUrl) => {
                try {
                    ...
                    if (previousSource && previousSource !== sourceUrl) {
                        throw new Error(
                            `Object-key collision with ${previousSource}`,
                        );
                    }
                    ...
                    const asset = {
                        ...
                        ...mapping,
                        contexts: [...contextsByAsset.get(sourceUrl)].sort(
                            compareUtf8,
                        ),
                        stagedPath,
                        bytes: response.body.byteLength,
                    ...
                    if (contentType.startsWith("text/css")) {
                        ...
                        for (const reference of extractCssReferences(
                            css,
                            sourceUrl,
                            options.sourceOrigin,
                        )) {
    ...
    manifest.assets = [...assetsByUrl.values()]
        .filter(Boolean)
        .sort((left, right) => compareUtf8(left.objectKey, right.objectKey));
    manifest.pages.sort((left, right) =>
        compareUtf8(left.sourceUrl, right.sourceUrl),
    );
    manifest.errors.sort((left, right) =>
        compareUtf8(
    ...
    if (!manifest.errors.length && options.upload) {
        ...
        const unchanged = uploadResults.filter(
    ...
    manifest.summary = {
        ...manifest.summary,
        pageCount: manifest.pages.length,
        storyPageCount: manifest.pages.filter((page) => page.type === "story")
            .length,
        remoteStoryCount: remoteStories.length,
        databaseIdolCount: idolIndex.size,
        assetCount: manifest.assets.length,
        storyMediaCount: manifest.assets.filter(
            (asset) => asset.kind === "story-media",
        ).length,
        staticAssetCount: manifest.assets.filter(
            (asset) => asset.kind === "wiki-static",
        ).length,
        totalAssetBytes: manifest.assets.reduce(
    ...
    if (manifest.errors.length) {
        throw new Error(
            `Wiki media sync has ${manifest.errors.length} error(s); see ${options.manifest}`,
        );
    }
...
async function runWikiMediaSync(options, dependencies = {}) {
    const openStoryRepository =
        dependencies.openStoryRepository || createPostgresStoryRepository;
    const resolveStorage = dependencies.resolveStorage || objectStorage;
    const closeStorage =
        dependencies.closeStorage || closeObjectStorageServices;
    const crawl = dependencies.syncWikiMedia || syncWikiMedia;
    const uploadExisting =
        dependencies.uploadExistingManifest || uploadExistingManifest;
    let storyRepository;
