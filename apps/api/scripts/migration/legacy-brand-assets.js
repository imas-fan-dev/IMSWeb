...
function parseArguments(argv, environment = process.env) {
    ...
    const defaultStaging = path.join(projectRoot, 'data/migration/legacy-brand-assets');
    const options = {
...
function validateR2Target(config, expectedBucket, expectEmptyPrefix) {
    if (config.type !== 's3') throw new Error('Brand asset acceptance requires S3 storage');
    if (config.bucket !== expectedBucket) {
        throw new Error(`R2 acceptance requires IMS_S3_BUCKET=${expectedBucket}`);
    }
    if (config.region !== 'auto') throw new Error('R2 acceptance requires IMS_S3_REGION=auto');
    let endpoint;
    try {
    ...
    if (config.forcePathStyle) throw new Error('R2 acceptance requires path-style addressing off');
...
function validateR2Acceptance(result) {
    if (result.some((asset) => asset.objectStatus !== 'unchanged')) {
        throw new Error('R2 brand asset acceptance failed because source and objects differ');
    }
    if (result.some((asset) => !asset.publicUrl || asset.publicStatus !== 200)) {
        throw new Error('R2 brand asset acceptance failed public URL verification');
    }
}
        ...
        try {
            const response = await fetch(url, {
                ...
                headers: {
                    ...
                    ...(method === 'HEAD' ? { 'Accept-Encoding': 'identity' } : {})
                },
                redirect: 'follow',
...
function validateTrueType(body) {
    ...
    const allowedSignatures = new Set([0x00010000, 0x4f54544f, 0x74727565, 0x74797031]);
    if (!allowedSignatures.has(signature)) throw new Error('IrisIdol is not an SFNT font');
    const tableCount = body.readUInt16BE(4);
    if (!tableCount || tableCount > 128 || 12 + tableCount * 16 > body.byteLength) {
    ...
    for (const requiredTag of ['head', 'maxp', 'name']) {
        if (!tags.has(requiredTag)) throw new Error(`IrisIdol is missing the ${requiredTag} table`);
    }
    return { tableCount };
}
...
async function assetDetails(definition, body) {
    ...
    if (metadata.format !== 'png' || !metadata.width || !metadata.height) {
        throw new Error(`${definition.sourcePath} must be a PNG with dimensions`);
    }
...
async function stageAssets(definitions, sourceBaseUrl, staging) {
    ...
    return Promise.all(definitions.map(async (definition, index) => {
        ...
        const response = await fetchWithRetry(sourceUrl, definition.sourcePath);
...
async function publicVerification(storage, entry) {
    const publicUrl = await storage.createPublicReadUrl?.(entry.objectKey) ?? null;
    ...
    if (contentType !== entry.contentType || contentLength !== entry.bytes) {
        throw new Error(`Public R2 metadata verification failed: ${entry.objectKey}`);
    }
    return { publicUrl, publicStatus: response.status };
}

async function syncObjects(storage, assets, apply, verifyPublic = publicVerification) {
    const results = [];
    for (const asset of assets) {
        const existing = await storage.get(asset.objectKey);
        const matches = existing !== null && existing.size === asset.bytes &&
        ...
        if (apply || matches) {
            ...
            if (!verified || verified.size !== asset.bytes || digest(verified.body) !== asset.sha256) {
                throw new Error(`Brand asset object verification failed: ${asset.objectKey}`);
            }
...
async function main() {
    ...
    if (storageConfig.type !== 's3') {
        throw new Error('Legacy brand asset migration requires IMS_OBJECT_STORAGE=s3');
    }
    if (options.requireR2) {
        validateR2Target(storageConfig, options.expectedBucket, options.expectEmptyPrefix);
    }
    const {
    ...
    try {
        ...
        const result = await syncObjects(migrationStorage.storage, assets, options.apply);
        ...
        const report = {
            ...
            summary: {
                ...
                imageCount: result.filter((asset) => asset.kind === 'image').length,
                fontCount: result.filter((asset) => asset.kind === 'font').length,
                totalBytes: result.reduce((total, asset) => total + asset.bytes, 0),
