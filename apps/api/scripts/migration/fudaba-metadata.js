...
const SOURCE_TABLE_SQL_SHA256 = new Map([
    [
        "card_favorites",
        "c2d9fb71ddfb5e973b34cde9d88ac4fb2a1f1a1e6186e8e9da7c3054ce9b28cb",
    ], // gitleaks:allow -- frozen schema SHA-256
    [
        "card_likes",
        "1a6bbb78a3bf4f0c6c35dc4bf30d4360c8fd9938839a853a623c41b151c7deff",
    ], // gitleaks:allow -- frozen schema SHA-256
    [
        "cards",
        "04e3d7ce4e5f9938eda7a2318dc280f247782e079ef0ae91ef602ad83e9f134b",
    ], // gitleaks:allow -- frozen schema SHA-256
    [
        "d1_migrations",
        "b635b9b3f4e2a717156ba8ba9a77dc0bae47407077a58e8c8c2d924113909e5c",
    ], // gitleaks:allow -- frozen schema SHA-256
    [
        "email_credentials",
        "f63aa9285cb02890422574a6882eb27367c592460ca3a9cfbca2b8a70945e75f",
    ], // gitleaks:allow -- frozen schema SHA-256
    [
        "exchange_requests",
        "6f83ad6cb7b1b0ff4c69e859f755902b223e5969fc6cc5ccdd2da917a3e064df",
    ], // gitleaks:allow -- frozen schema SHA-256
    [
        "messages",
        "2d37acefaf22ac92076af995085ab3b78060306f36972912d24ff0b873a44adf",
    ], // gitleaks:allow -- frozen schema SHA-256
    [
        "oauth_accounts",
        "852dba401f3c770479991cdf2ac9e884a75934868dcd612f5653c63620c46383",
    ], // gitleaks:allow -- frozen schema SHA-256
    [
        "oauth_states",
        "40e144eeb015b4203ac822f63243352e0896993592323a8baa6f165664a864ab",
    ], // gitleaks:allow -- frozen schema SHA-256
    [
        "office_cards",
        "f5ed861f98bb262df36f4f2e99ac3d1d4ba25b1dea3989001ae428142b4122d4",
    ], // gitleaks:allow -- frozen schema SHA-256
    [
        "office_series_tags",
        "49df569b5bd6f488f1c7eac16ab1d70f28cdbc97d023076af7861ae40927c614",
    ], // gitleaks:allow -- frozen schema SHA-256
    [
        "offices",
        "ceae918f48c721ef183a3d3774ef7256a63662cbf4666cbcb39aafe61ed6b52b",
    ], // gitleaks:allow -- frozen schema SHA-256
    [
        "series_tags",
        "e4f37bc4013605d08ff85365a61718368ebac3f16a839a24adbc51f512ae2103",
    ], // gitleaks:allow -- frozen schema SHA-256
    [
        "sessions",
        "328f61103400b61613e94679f886b0decd852c4222b14bb4f21dc6f5818be734",
    ], // gitleaks:allow -- frozen schema SHA-256
    [
        "users",
        "ecc2c0a0bea537f7882dde3e0e51a5110dd1dfca896e47e7932eeaa1baf61f99",
    ], // gitleaks:allow -- frozen schema SHA-256
]);
...
function validateSnapshotId(value) {
    if (typeof value !== "string" || !SNAPSHOT_ID_PATTERN.test(value)) {
        throw new Error(
            "Snapshot ID must match [A-Za-z0-9][A-Za-z0-9._-]{0,79}",
        );
    }
    return value;
}
...
function assertNoSqliteSidecars(filename) {
    for (const suffix of ["-wal", "-shm", "-journal"]) {
        if (fs.existsSync(`${filename}${suffix}`)) {
            throw new Error(
                `SQLite sidecar must be absent: ${filename}${suffix}`,
            );
        }
    }
}
...
async function validateSourceDatabase(filename) {
    ...
    try {
        ...
        const foreignKeys = await sqliteAll(
            database,
        ...
        const unexpectedTables = [...tableNames].filter(
            (table) =>
                !SOURCE_TABLES[table] && !ALLOWED_D1_INTERNAL_TABLES.has(table),
        );
        if (unexpectedTables.length) {
            throw new Error(
                `Unexpected Fudaba source table(s): ${unexpectedTables.sort().join(",")}`,
            );
        }
        if (!tableNames.has("d1_migrations")) {
            throw new Error(
                "Missing Fudaba D1 migration ledger: d1_migrations",
            );
        }
        for (const [table, definition] of Object.entries(SOURCE_TABLES)) {
            if (!tableNames.has(table))
                throw new Error(`Missing Fudaba source table: ${table}`);
            ...
            for (const column of columns) {
                ...
                const expectedType = SOURCE_REAL_COLUMNS.has(identity)
                    ...
                    : SOURCE_INTEGER_COLUMNS.has(identity)
                      ? "INTEGER"
                      : "TEXT";
                const expectedNotNull = SOURCE_NULLABLE_COLUMNS.has(identity)
                    ? 0
                    : 1;
                const expectedPrimaryKey =
                    definition.key.indexOf(column.name) + 1;
                const expectedDefault =
                    SOURCE_COLUMN_DEFAULTS.get(identity) || null;
                if (
                    String(column.type).toUpperCase() !== expectedType ||
                    Number(column.notnull) !== expectedNotNull ||
                    Number(column.pk) !== expectedPrimaryKey ||
                    (column.dflt_value === null
                        ? null
                        : String(column.dflt_value)) !== expectedDefault
                ) {
                    throw new Error(
        ...
        const tableSql = new Map(
            tables.map((table) => [table.name, table.sql]),
        );
        for (const [table, expectedSha256] of SOURCE_TABLE_SQL_SHA256) {
            const actualSha256 = sha256(
                normalizeSchemaSql(tableSql.get(table)),
            );
            if (actualSha256 !== expectedSha256) {
                throw new Error(`Fudaba source table DDL drift: ${table}`);
            }
        }
        const ledger = await sqliteAll(
            database,
            "SELECT name FROM d1_migrations ORDER BY id",
        );
        if (
            JSON.stringify(ledger.map((row) => row.name)) !==
            JSON.stringify(FUDABA_MIGRATIONS)
        ) {
            throw new Error(
                "Fudaba D1 migration ledger does not match the locked migration list",
            );
        }

        const explicitIndexes = await sqliteAll(
            database,
            `
            SELECT name, sql FROM sqlite_master
            WHERE type = 'index' AND sql IS NOT NULL
              AND tbl_name IN (${Object.keys(SOURCE_TABLES)
                  .map(() => "?")
                  .join(", ")})
            ORDER BY name
        `,
        ...
        if (
            ...
            throw new Error(
                "Fudaba source indexes do not match the locked schema",
            );
        }
        ...
        const triggers = await sqliteAll(
            database,
            `
        ...
        if (
            ...
            throw new Error(
                "Fudaba source triggers do not match the locked schema",
            );
        }
        for (const trigger of triggers) {
            ...
            if (actualSha256 !== SOURCE_TRIGGER_SQL_SHA256.get(trigger.name)) {
                throw new Error(
                    `Fudaba source trigger DDL drift: ${trigger.name}`,
                );
            }
        }
...
function sourceManifestKey(table, row) {
    ...
    if (definition.sensitiveKey) {
        return {
            sha256: sha256(
                definition.key.map((field) => row[field]).join("\0"),
            ),
        };
    }
    return Object.fromEntries(
        definition.key.map((field) => [field, row[field]]),
    );
}

function sourceRowIdentity(table, rowOrKey) {
    const key =
        Object.hasOwn(rowOrKey, "classification") ||
        Object.hasOwn(rowOrKey, "rowSha256")
            ? rowOrKey.key
...
async function readSourceRows(filename) {
    ...
    try {
        ...
        const output = {};
        for (const [table, definition] of Object.entries(SOURCE_TABLES)) {
            const order = definition.key
                .map(
                    (column) =>
                        `CAST(${quoteSqliteIdentifier(column)} AS BLOB)`,
                )
                .join(", ");
...
async function redactOperationalRows(filename) {
    ...
    if ([...NON_IMPORTED_TABLES].some((table) => counts[table] !== 0)) {
        throw new Error(
            "Operational authentication rows were not fully redacted",
        );
    }
}
...
function classificationIndex(input) {
    ...
    for (const entry of classificationEntries(input)) {
        ...
        if (!SOURCE_CLASSIFICATIONS.has(entry.classification)) {
            throw new Error(
                `Invalid source classification: ${entry.classification}`,
            );
        }
        if (
            !SOURCE_TABLES[entry.table] ||
            NON_IMPORTED_TABLES.has(entry.table)
        ) {
            throw new Error(
                `Classification entry targets a non-importable table: ${entry.table}`,
            );
        }
        if (
            entry.verifiedProductionCount === true &&
            (entry.table !== "offices" ||
                typeof entry.productionCountEvidence !== "string" ||
        ...
        if (index.has(identity))
            throw new Error(`Duplicate classification entry: ${identity}`);
        index.set(identity, {
...
function buildRowsManifest(
    ...
    const summary = {
        ...
        classifications: Object.fromEntries(
            [...SOURCE_CLASSIFICATIONS].map((value) => [value, 0]),
        ),
    };
    for (const [table, tableRows] of Object.entries(rows)) {
        ...
        tables[table] = tableRows.map((row) => {
            ...
            if (classificationByRow.has(identity))
                usedClassifications.add(identity);
            summary.total += 1;
            ...
            if (table === "offices") {
                descriptor.verifiedProductionCount =
                    selected.verifiedProductionCount;
                if (selected.verifiedProductionCount) {
                    descriptor.productionCountEvidence =
                        selected.productionCountEvidence;
                }
            }
            ...
            if (
                table === "users" &&
                selected.classification === "owner-approved-reference"
            ) {
...
async function extractSnapshot(options) {
    ...
    for (const [field, value] of [
        ...
        if (typeof value !== "string" || !value || value !== value.trim()) {
            throw new Error(
                `${field} is required and must not contain surrounding whitespace`,
            );
        }
    }
    ...
    const snapshotRoot = path.resolve(
        options.snapshotRoot || DEFAULT_SNAPSHOT_ROOT,
    );
    ...
    try {
        ...
        if (sha256File(source) !== sourceExportSha256) {
            throw new Error(
                "Fudaba source database content changed while copying",
            );
        }
        ...
        if (fs.existsSync(target)) {
            ...
            if (sha256File(target) !== databaseSha256) {
                throw new Error(
                    `Snapshot already exists with different database: ${snapshotId}`,
                );
            }
            fs.rmSync(temporary);
        } else {
        ...
        if (typeof classifications === "string") {
            classifications = readJsonFile(
                path.resolve(classifications),
                "classifications",
            );
        } else if (
            !classifications &&
            fs.existsSync(path.join(snapshotDirectory, "rows-manifest.json"))
        ) {
            const previous = readJsonFile(
                path.join(snapshotDirectory, "rows-manifest.json"),
                "existing rows manifest",
            );
            if (previous.sourceSha256 === sourceExportSha256)
        ...
        if (fs.existsSync(sourceManifestFile)) {
            const existingSource = readJsonFile(
                sourceManifestFile,
                "existing source manifest",
            );
            if (canonicalHash(existingSource) !== canonicalHash(sourceJson)) {
                throw new Error(
                    `Snapshot metadata is immutable once created: ${snapshotId}`,
                );
            }
        }
        ...
        for (const [filename, placeholder] of [
            [
                "media-manifest.json",
                emptyMediaManifest(snapshotId, sourceExportSha256),
            ],
            [
                "rights-manifest.json",
                emptyRightsManifest(snapshotId, sourceExportSha256),
            ],
            [
                "reconciliation.json",
                {
...
function parseTimestamp(value, label) {
    ...
    if (
        /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}(?:\.\d{1,3})?$/.test(normalized)
    ) {
    ...
    if (zone !== "Z") {
        ...
        if (zoneHour > 23 || zoneMinute > 59)
            throw new Error(`${label} has invalid timezone`);
    }
...
function manifestEntries(manifest, property) {
    ...
    if (property === "approvals" && Array.isArray(manifest?.entries))
...
function indexManifestEntries(entries, label) {
    ...
    for (const entry of entries) {
        ...
        if (
            entityKind(entry) === undefined ||
            entityId(entry) === undefined ||
            entitySlot(entry) === undefined ||
            index.has(identity)
        ) {
            throw new Error(
                `${label} has invalid or duplicate entry: ${identity}`,
            );
        }
...
function mediaObjectKey(entry) {
    ...
    if (Object.hasOwn(entry, "logical_object_key"))
        return entry.logical_object_key;
...
function assertLogicalMediaKey(kind, id, slot, logicalKey) {
    if (
        ...
        throw new Error(
            `Media entity ID is not a safe object-key segment: ${String(id)}`,
        );
    }
    const bases = {
...
function resolveMedia(
    ...
    if (
        state !== "ready" ||
        media.disposition !== "store-protected" ||
        media.storageScope !== "private" ||
        typeof media.targetBucket !== "string" ||
        !media.targetBucket ||
        typeof media.objectId !== "string" ||
        !media.objectId ||
        typeof media.physicalObjectKey !== "string" ||
        !media.physicalObjectKey ||
        !/(^|\/)__protected\//.test(media.physicalObjectKey) ||
        typeof media.targetEtag !== "string" ||
        !media.targetEtag ||
        !Number.isSafeInteger(media.bytes) ||
        media.bytes < 1 ||
        !expectedExtension ||
        !logicalKey.endsWith(`.${expectedExtension}`) ||
    ...
    const approval =
        rights?.approvalStatus || rights?.rightsStatus || rights?.status;
    if (
        approval !== "approved" ||
        rights?.action !== "store-protected" ||
        mediaObjectKey(rights) !== logicalKey ||
        rights?.bindingSha256 !== binding ||
        rights?.sourceReference !== source ||
        rights?.sourceSha256 !== digest ||
        rights?.bytes !== media.bytes ||
        rights?.contentType !== media.contentType ||
        !SHA256_PATTERN.test(rights?.evidenceSha256 || "") ||
        typeof rights?.reviewedBy !== "string" ||
...
function resolveOptionalMedia(
    ...
    if (
        state === "external" &&
        media.disposition === "retain-external" &&
        allowExternal &&
        approval === "approved" &&
        rights.action === "retain-external" &&
        mediaObjectKey(media) === null &&
        mediaObjectKey(rights) === null &&
        media.externalUrl === source &&
        /^https:\/\//.test(source) &&
        SHA256_PATTERN.test(rights.evidenceSha256 || "") &&
        typeof rights.reviewedBy === "string" &&
        rights.reviewedBy.trim()
...
function targetKey(operation) {
    return Object.fromEntries(
        operation.keyColumns.map((field) => [field, operation.row[field]]),
    );
}
    ...
    switch (operation.table) {
        case "platform_accounts":
            ...
            if (BigInt(row.updated_at) < BigInt(row.created_at)) {
                throw new Error(
                    "platform account updated_at precedes created_at",
                );
            }
            break;
        case "platform_profiles":
            ...
            if (row.avatar_external_url !== null) {
                assertText(
                    row.avatar_external_url,
                    "profile avatar external URL",
                    1,
                    2048,
                );
            }
            if (
                ...
                row.avatar_external_url !== null
            ) {
                throw new Error(
                    "profile cannot have both avatar object key and external URL",
                );
            }
            if (row.home_city !== null)
                assertText(row.home_city, "profile home_city", 0, 100);
        ...
        case "platform_oauth_identities":
            ...
            if (BigInt(row.updated_at) < BigInt(row.created_at)) {
                throw new Error(
                    "OAuth identity updated_at precedes created_at",
                );
            }
            break;
        case "platform_email_credentials":
            ...
            if (!/^[a-f0-9]{64}$/.test(row.password_hash)) {
                throw new Error(
                    "PBKDF2 password hash must be 32-byte lowercase hex",
                );
            }
            assertText(row.salt, "credential salt", 1, 1024);
            if (BigInt(row.updated_at) < BigInt(row.created_at)) {
                throw new Error(
                    "email credential updated_at precedes created_at",
                );
            }
            break;
        case "fudaba_offices":
            ...
            if (!/^#[0-9A-Fa-f]{6}$/.test(row.accent))
                throw new Error("office accent is invalid");
            if (row.cover_object_key !== null) {
                assertText(
                    row.cover_object_key,
                    "office cover object key",
                    1,
                    1024,
                );
            }
            if (
                row.pending_cover_object_key !== null ||
                row.pending_cover_submitted_at !== null
            ) {
                throw new Error("imported office cannot carry a pending cover");
            }
            if (
                BigInt(row.visitor_count) < 0n ||
                BigInt(row.visitor_count) > 9007199254740991n
            ) {
            ...
            if (
                row.archived_at &&
                Date.parse(row.archived_at) < Date.parse(row.created_at)
            ) {
        ...
        case "fudaba_cards":
            ...
            if (!/^#[0-9A-Fa-f]{6}$/.test(row.accent))
                throw new Error("card accent is invalid");
            ...
            if (row.source_label !== null)
                assertText(row.source_label, "source label", 0, 200);
            if (row.source_credit !== null)
                assertText(row.source_credit, "source credit", 0, 200);
        ...
        case "fudaba_office_series_tags":
            ...
            if (row.display_order < 0)
                throw new Error("office series display_order is negative");
            break;
        case "fudaba_office_cards":
            ...
            if (
                ...
                throw new Error(
                    "office card placement is outside target constraints",
                );
            }
            if (row.revision !== 0 || row.updated_at !== row.pinned_at) {
                throw new Error(
                    "imported office card has invalid initial revision",
                );
            }
            break;
        ...
        case "fudaba_exchange_requests":
            ...
            if (row.requester_account_id === row.recipient_account_id) {
                throw new Error(
                    "exchange requester and recipient are identical",
                );
            }
            if (row.offered_card_id === row.wanted_card_id) {
                throw new Error(
                    "exchange offered and wanted cards are identical",
                );
            }
...
function verifyArtifactIdentity(manifest, sourceJson, label) {
    if (
        manifest.snapshotId !== sourceJson.snapshotId ||
...
function verifyMediaPlan(
    ...
    if (
        ...
        plan.sourceCommit !== FUDABA_COMMIT ||
    ...
    for (const [manifest, property, label] of [
        ...
        const entries = indexManifestEntries(
            manifestEntries(manifest, property),
            label,
        );
        if (entries.size !== planEntries.size) {
            throw new Error(
                `${label} entry count does not match the media plan`,
            );
        }
        for (const [identity, planned] of planEntries) {
            ...
            if (
                !entry ||
                entry.bindingSha256 !== planned.bindingSha256 ||
                sourceReference(entry) !== sourceReference(planned) ||
                mediaObjectKey(entry) !== mediaObjectKey(planned)
            ) {
                throw new Error(
                    `${label} entry does not match the media plan: ${identity}`,
                );
            }
        }
    }
...
async function loadSnapshot(snapshotDirectory) {
    ...
    if (
        ...
        sourceJson.source?.commit !== FUDABA_COMMIT ||
        JSON.stringify(sourceJson.source?.migrations) !==
            JSON.stringify(FUDABA_MIGRATIONS)
    ) {
        throw new Error(
            "Source manifest does not match the locked Fudaba commit/migrations",
        );
    }
    if (
        sourceJson.source?.d1DatabaseId !== FUDABA_D1_DATABASE_ID ||
    ...
    if (
        sourceJson.database?.sha256 !== databaseSha256 ||
        sourceJson.database?.bytes !== stat.size
    ) {
        throw new Error(
            "Snapshot database size or SHA-256 does not match source.json",
        );
    }
    ...
    if (
        mediaManifest.schemaVersion !== 2 ||
        rightsManifest.schemaVersion !== 2
    ) {
    ...
    for (const table of NON_IMPORTED_TABLES) {
        if (rows[table].length !== 0) {
            throw new Error(
                `Snapshot database retains operational authentication rows: ${table}`,
            );
        }
        ...
        if (
            !validation ||
            !Number.isSafeInteger(validation.sourceCount) ||
            validation.sourceCount < 0 ||
            validation.snapshotCount !== 0 ||
            validation.redacted !== true ||
            !manifest ||
            manifest.count !== validation.sourceCount ||
            manifest.migrated !== false ||
...
function descriptorIndex(rowsManifest) {
    if (
        rowsManifest.schemaVersion !== 1 ||
        !rowsManifest.tables ||
        typeof rowsManifest.tables !== "object" ||
        Array.isArray(rowsManifest.tables)
    ) {
        throw new Error("Invalid rows manifest");
    }
    const manifestTables = Object.keys(rowsManifest.tables);
    const unexpectedTables = manifestTables.filter(
        (table) => !SOURCE_TABLES[table],
    );
    ...
    for (const [table, rows] of Object.entries(rowsManifest.tables || {})) {
        ...
        for (const descriptor of rows) {
            if (!SOURCE_CLASSIFICATIONS.has(descriptor.classification)) {
                throw new Error(
                    `Invalid classification in rows manifest: ${descriptor.classification}`,
                );
            }
            if (
                table === "offices" &&
                descriptor.verifiedProductionCount === true &&
                (typeof descriptor.productionCountEvidence !== "string" ||
                    !descriptor.productionCountEvidence.trim())
            ) {
                throw new Error(
                    "Verified office visitor count is missing evidence",
                );
            }
            const identity = `${table}:${JSON.stringify(canonicalize(descriptor.key))}`;
            if (index.has(identity))
                throw new Error(`Duplicate rows manifest entry: ${identity}`);
    ...
    switch (table) {
        ...
        case "oauth_accounts":
            ...
            if (context.isReferenceAccount(row.user_id)) {
                throw new Error(
                    "owner-approved-reference OAuth identity must not be imported",
                );
            }
        ...
        case "email_credentials":
            if (context.isReferenceAccount(row.user_id)) {
                throw new Error(
                    "owner-approved-reference credential must not be imported",
                );
            }
            context.requireIncluded("users", row.user_id);
            return [
        ...
        case "offices": {
            ...
            const archived = optionalTimestamp(
                row.archived_at,
                "offices archived_at",
            );
            ...
            if (
                sourceVisitorCount < 0 ||
                sourceVisitorCount > Number.MAX_SAFE_INTEGER
            ) {
            ...
            if (
                ...
                throw new Error(
                    "verified visitor count requires productionCountEvidence",
                );
            }
            const visitorCount =
                descriptor.verifiedProductionCount === true
                    ? sourceVisitorCount
                    : 0;
            context.reportsByIdentity.get(
                sourceRowIdentity(table, row),
            ).visitorCount = {
                source: sourceVisitorCount,
                imported: visitorCount,
                verifiedProductionCount:
                    descriptor.verifiedProductionCount === true,
        ...
        case "cards": {
            ...
            return [
                targetOperation(table, sourceKey, "fudaba_cards", ["id"], {
                    id: row.id,
                    owner_account_id: context.accountId(row.owner_id),
        ...
        case "messages":
            ...
            return [
                targetOperation(table, sourceKey, "fudaba_messages", ["id"], {
                    id: row.id,
                    office_id: row.office_id,
        ...
        case "exchange_requests": {
            ...
            if (row.offered_card_id !== null)
                context.requireIncluded("cards", row.offered_card_id);
            context.requireIncluded("office_cards", {
                office_id: row.office_id,
                card_id: row.wanted_card_id,
            });
            if (
                context.cardOwner(row.wanted_card_id) !==
                context.accountId(row.recipient_id)
            ) {
                throw new Error(
                    "wanted card is not owned by exchange recipient",
                );
            }
            if (
                row.offered_card_id !== null &&
                context.cardOwner(row.offered_card_id) !==
                    context.accountId(row.requester_id)
            ) {
                throw new Error(
                    "offered card is not owned by exchange requester",
                );
            }
            if (
            ...
            if (
                row.status !== "pending" &&
                descriptor.verifiedFinalStatus !== true
            ) {
                throw new Error(
                    "final exchange status requires verifiedFinalStatus approval",
                );
            }
            ...
            return [
                targetOperation(
                    ...
                    ["id"],
                    {
                        ...
                        recipient_account_id: context.accountId(
                            row.recipient_id,
                        ),
...
function sourceTableConservation(reports, sourceCounts) {
    ...
    for (const table of Object.keys(SOURCE_TABLES)) {
        const matching = reports.filter(
            (report) => report.sourceTable === table,
        );
        const count = (outcome) =>
            matching
...
async function buildImportPlan(snapshotDirectory) {
    ...
    for (const [table, rows] of Object.entries(snapshot.rows)) {
        ...
        for (const row of rows) {
            ...
            if (!descriptor || descriptor.rowSha256 !== canonicalHash(row)) {
                throw new Error(
                    `Rows manifest does not match source row: ${identity}`,
                );
            }
            const report = rowReport(table, descriptor);
            reports.push(report);
            reportsByIdentity.set(identity, report);
            if (INCLUDED_CLASSIFICATIONS.has(descriptor.classification))
                includedRows.add(identity);
            if (
                table === "users" &&
                INCLUDED_CLASSIFICATIONS.has(descriptor.classification)
            ) {
                if (descriptor.classification === "owner-approved-reference") {
                    if (
                        typeof descriptor.targetAccountId !== "string" ||
                        descriptor.targetAccountId.length < 1 ||
                        descriptor.targetAccountId.length > 128
                    ) {
                        report.outcome = "failed";
                        report.reasonCode =
                            "owner-approved-reference-requires-target-account";
                        report.reason =
                            "owner-approved-reference-requires-targetAccountId";
    ...
    const importedSourceCount = reports.filter((report) =>
        Object.hasOwn(report, "sourceKey"),
    ).length;
    if (descriptors.size !== importedSourceCount) {
        throw new Error(
            "Rows manifest contains entries not present in database.sqlite",
        );
    }
    ...
    for (const user of snapshot.rows.users) {
        ...
        if (
            descriptor.classification === "production-user-content" &&
    ...
    const context = {
        ...
        accountId(sourceAccountId) {
            ...
            if (!targetAccountId) {
                throw new Error(
                    `Source account has no target mapping: ${sourceAccountId}`,
                );
            }
        ...
        cardOwner(cardId) {
            const card = snapshot.rows.cards.find(
        ...
        requireIncluded(table, key) {
            ...
            if (!includedRows.has(identity))
                throw new Error(`Required source row is excluded: ${identity}`);
        },
    };
    const operations = [];
    ...
    const unusedMedia = [...mediaIndex.keys()].filter(
        (identity) => !consumedMedia.has(identity),
    );
    const unusedRights = [...rightsIndex.keys()].filter(
        (identity) => !consumedMedia.has(identity),
    );
    ...
    for (const operation of operations.filter(
        (item) => item.table === "platform_email_credentials",
    )) {
        ...
        if (existing) {
            for (const candidate of [existing, operation]) {
                ...
                candidate.sourceReport.reasonCode =
    ...
    markUniqueConflicts(
        operations.filter(
            (item) =>
                item.table === "fudaba_exchange_requests" &&
    ...
    for (const operation of operations) {
        ...
        if (
            existing &&
            canonicalHash(existing.row) !== canonicalHash(operation.row)
        ) {
    ...
    const mediaTargets = manifestEntries(snapshot.mediaManifest, "entries")
        .filter(
            (entry) =>
                entry.state === "ready" &&
                entry.disposition === "store-protected",
        )
        .map((entry) => ({
    ...
    const targetBuckets = new Set(
        mediaTargets.map((target) => target.targetBucket),
    );
    if (targetBuckets.size > 1) {
        throw new Error(
            "Fudaba media manifest spans more than one target bucket",
        );
    }
...
function normalizedPgValue(table, column, value) {
    ...
    if (PG_BIGINT_COLUMNS.has(`${table}.${column}`) && value !== null)
        return String(value);
    return value;
}

function differingColumns(operation, actual) {
    return operation.compareColumns.filter(
        (column) =>
            canonicalHash(
...
function projectedOperationRow(operation) {
    return Object.fromEntries(
        operation.compareColumns.map((column) => [
            column,
...
async function inspectMediaControlPlane(client, expected, lock = false) {
    ...
    const differentColumns = Object.keys(expectedProjection).filter(
        (column) =>
            canonicalHash(actualProjection[column]) !==
            canonicalHash(expectedProjection[column]),
    );
    return {
...
function assertMediaTargetBucket(plan, options) {
    ...
    if (options.targetBucket !== plan.mediaTargetBucket) {
        throw new MigrationBlockedError(
            "Import requires exact --target-bucket",
            {
...
async function inspectOperation(client, operation) {
    ...
    const expectedTargetSha256 = canonicalHash(
        projectedOperationRow(operation),
    );
    if (!result.rows.length) {
        for (const columns of alternativeUniqueKeys(operation)) {
            const uniqueWhere = columns
                .map(
                    (column, index) =>
                        `${quotePgIdentifier(column)} = $${index + 1}`,
                )
                .join(" AND ");
    ...
    const actualTargetSha256 = canonicalHash(
        projectedActualRow(operation, result.rows[0]),
    );
...
async function transitionOperation(client, operation, fromRow, toRow) {
    const changedColumns = operation.compareColumns.filter(
        (column) =>
            canonicalHash(
                normalizedPgValue(operation.table, column, fromRow[column]),
            ) !==
            canonicalHash(
                normalizedPgValue(operation.table, column, toRow[column]),
            ),
    );
    ...
    if (result.rowCount !== 1) {
        throw new Error(
            `Target changed while staging import: ${operation.table}`,
        );
    }
}
...
async function compareOrImport(plan, options = {}) {
    ...
    const markRolledBack = () => {
        const rolledBackInserts = report.targets.filter(
            (target) => target.state === "inserted",
        );
    ...
    const refreshRowSummary = () => {
        report.sourceTables = sourceTableConservation(
            report.rows,
            plan.sourceCounts,
        );
        Object.assign(
            report.summary,
            summarizeSourceTables(report.sourceTables),
        );
        report.targetTables = targetTableSummary(
            plan.operations,
            report.targets,
        );
    };
    try {
        ...
        await client.query(
            "SELECT pg_advisory_lock(hashtext('imsweb-fudaba-metadata-import'))",
        );
        advisoryLocked = true;
        await client.query(
            apply
        ...
        for (const expected of plan.mediaTargets) {
            const inspection = await inspectMediaControlPlane(
                client,
                expected,
                apply,
            );
        ...
        for (const operation of plan.operations) {
            ...
            if (inspection.state === "conflict") {
                ...
                if (operation.requiredExisting) {
                    ...
                    operation.sourceReport.outcome = "failed";
                    operation.sourceReport.reasonCode =
                        "required-existing-target-missing";
                    operation.sourceReport.reason = `required-existing-target-missing:${operation.table}`;
                    report.summary.conflicts += 1;
                ...
                if (apply) {
                    try {
                        ...
                        const verification = await inspectOperation(
                            client,
                            writtenOperation,
                        );
                        ...
                        if (operation.stagedRow) {
                            ...
                            target.actualTargetSha256 =
                                verification.actualTargetSha256;
                        }
                    } catch (error) {
                        operation.sourceReport.outcome = "failed";
                        operation.sourceReport.reasonCode =
        ...
        throw new MigrationBlockedError(
            `Fudaba import failed: ${error.message}`,
            report,
        );
    } finally {
...
async function importSnapshot(options) {
    ...
    if (options.apply) {
        if (options.confirmSnapshotId !== plan.snapshot.sourceJson.snapshotId) {
            throw new MigrationBlockedError(
                "Apply requires exact --confirm-snapshot-id",
                {
                    snapshotId: plan.snapshot.sourceJson.snapshotId,
                    sourceSha256: plan.snapshot.sourceJson.sourceExport.sha256,
                },
        ...
        if (
            ...
            plan.snapshot.sourceJson.sourceExport.sha256
        ) {
            throw new MigrationBlockedError(
                "Apply requires exact --confirm-source-sha256",
                {
                    snapshotId: plan.snapshot.sourceJson.snapshotId,
                    sourceSha256: plan.snapshot.sourceJson.sourceExport.sha256,
                },
    ...
    try {
        ...
        if (
            ...
            options.apply !== true ||
            error.report?.outcomeUnknown !== true
        ) {
            throw error;
        }
        try {
...
async function reconcileSnapshot(options) {
    ...
    if (!plan.blockers.length) {
        const pool = poolFor(
            options.connectionString,
            "imsweb-fudaba-metadata-reconcile",
        );
        let client;
        let advisoryLocked = false;
        let transactionOpen = false;
        try {
            ...
            for (const expected of plan.mediaTargets) {
                ...
                report.summary[
                    `media${inspection.state[0].toUpperCase()}${inspection.state.slice(1)}`
                ] += 1;
                report.mediaTargets.push({
                    identity: expected.identity,
                    logicalObjectKey: expected.logicalObjectKey,
    ...
    report.status =
        ...
        report.summary.mediaMissing ||
        report.summary.mediaDifferent
            ? "failed"
            : "passed";
    report.targetTables = targetTableSummary(plan.operations, report.targets);
    if (options.write !== false) {
        writeJsonAtomic(
            path.join(plan.snapshot.directory, "reconciliation.json"),
            report,
        );
    }
    return report;
...
function resolveSnapshotDirectory(options) {
    if (options.snapshotDirectory)
        return path.resolve(options.snapshotDirectory);
    if (!options.snapshotId)
        throw new Error("--snapshot or --snapshot-id is required");
    validateSnapshotId(options.snapshotId);
    return path.join(
        path.resolve(options.snapshotRoot || DEFAULT_SNAPSHOT_ROOT),
        options.snapshotId,
    );
}
...
function extractHelp() {
    return (
        "Usage: fudaba-extract.js --source SQLITE --snapshot-id ID " +
...
function importHelp() {
    return (
        "Usage: fudaba-import.js (--snapshot DIRECTORY | --snapshot-id ID) " +
...
function reconcileHelp() {
    return (
        "Usage: fudaba-reconcile.js (--snapshot DIRECTORY | --snapshot-id ID) " +
...
async function runMain(action, help, argv, environment) {
    try {
        ...
        if (action === extractSnapshot) {
            if (!options.source || !options.snapshotId)
                throw new Error(extractHelp());
        } else {
        ...
        if (action === reconcileSnapshot && report.status !== "passed")
            process.exitCode = 1;
        return report;
    } catch (error) {
        if (error.report)
