...
function normalizedBaseUrl(value) {
    ...
    if (!['http:', 'https:'].includes(parsed.protocol) || parsed.username || parsed.password) {
        throw new Error('--source-base-url must be an HTTP(S) URL without credentials');
    }
    parsed.hash = '';
    parsed.search = '';
...
function parseArguments(argv, environment = process.env) {
    ...
    const defaultStaging = path.join(projectRoot, 'data/migration/legacy-namecards');
    const options = {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (argument === '--source-base-url') options.sourceBaseUrl = normalizedBaseUrl(next());
        else if (argument === '--staging-dir') options.staging = path.resolve(next());
...
function sourceTimestamp(value) {
    ...
    if (!Number.isFinite(Date.parse(iso))) throw new Error(`Invalid Legacy namecard timestamp: ${value}`);
    return new Date(iso).toISOString();
}

function sourceMediaPath(value) {
    if (typeof value !== 'string' || !value.startsWith('/uploads/namecard/original/')) {
        throw new Error(`Invalid Legacy namecard media URL: ${String(value)}`);
    }
    const parsed = new URL(value, 'https://legacy.invalid');
    if (parsed.origin !== 'https://legacy.invalid' || parsed.search || parsed.hash ||
...
function normalizeCard(value) {
    if (!value || typeof value !== 'object') throw new Error('Legacy namecard row must be an object');
    const id = Number(value.id);
    if (!Number.isSafeInteger(id) || id < 1) throw new Error(`Invalid Legacy namecard ID: ${value.id}`);
    ...
    if (value.status !== 'approved') throw new Error(`Legacy public namecard ${id} is not approved`);
    return {
        id,
...
async function fetchWithRetry(url, options) {
    ...
    for (let attempt = 1; attempt <= 4; attempt += 1) {
        try {
            ...
            lastError = new Error(`${options.label} returned HTTP ${response.status}`);
        } catch (error) {
            lastError = error;
        }
        if (attempt < 4) {
...
async function fetchLegacyCards(sourceBaseUrl) {
    ...
    for (let page = 1; ; page += 1) {
        ...
        if (!payload || !Array.isArray(payload.list) || !Number.isSafeInteger(Number(payload.total))) {
            throw new Error(`Legacy namecard page ${page} has an invalid response shape`);
        }
        const total = Number(payload.total);
        if (expectedTotal === null) expectedTotal = total;
        if (expectedTotal !== total) throw new Error('Legacy namecard total changed during export');
        cards.push(...payload.list.map(normalizeCard));
        if (!payload.list.length || cards.length >= total) break;
    }
    if (cards.length !== expectedTotal) {
        throw new Error(`Legacy namecard export returned ${cards.length} of ${expectedTotal} rows`);
    }
...
function normalizeReactions(value, cardId) {
    if (!value || typeof value !== 'object' || Array.isArray(value)) {
        throw new Error(`Legacy reactions for card ${cardId} must be an object`);
    }
    return Object.entries(value).map(([emoji, count]) => {
        const normalizedCount = Number(count);
        if (!emoji || emoji.length > 32 || !Number.isSafeInteger(normalizedCount) || normalizedCount < 1) {
            throw new Error(`Legacy reactions for card ${cardId} contain an invalid value`);
        }
...
async function imageDetails(body, cardId, side) {
    if (!body.byteLength || body.byteLength > MAX_IMAGE_BYTES) {
        throw new Error(`Legacy namecard ${cardId} ${side} has an invalid byte size`);
    }
    let metadata;
    try {
        ...
        throw new Error(`Legacy namecard ${cardId} ${side} is not a decodable image`);
    }
    const format = IMAGE_FORMATS.get(metadata.format);
    if (!format || !metadata.width || !metadata.height) {
        throw new Error(`Legacy namecard ${cardId} ${side} uses an unsupported image format`);
    }
    return { ...format, width: metadata.width, height: metadata.height };
}

function targetFilename(cardId, side, extension) {
    if (!Number.isSafeInteger(cardId) || cardId < 1 || !['front', 'back'].includes(side) ||
...
async function stageMedia(sourceBaseUrl, mediaDirectory, input) {
    const cached = await cachedMedia(mediaDirectory, input.cardId, input.side, input.expectedMd5);
    if (cached) return canonicalMedia(input, cached);
    let stagedFrom = 'download';
    let response = await fetchWithRetry(new URL(input.sourcePath, sourceBaseUrl), {
        timeoutMs: 90_000,
    ...
    if (response.status === 404 && input.sourcePath.includes('%')) {
        ...
        response = await fetchWithRetry(new URL(encodedPercentPath, sourceBaseUrl), {
            timeoutMs: 90_000,
    ...
    if (!response.ok) {
        throw new Error(`Legacy namecard ${input.cardId} ${input.side} returned HTTP ${response.status}`);
    }
...
async function stageAllMedia(sourceBaseUrl, staging, cards) {
    const inputs = cards.flatMap((card) => [
        { cardId: card.id, side: 'front', sourcePath: card.sourceImage1Url, expectedMd5: card.hash1 },
        { cardId: card.id, side: 'back', sourcePath: card.sourceImage2Url, expectedMd5: card.hash2 }
    ]);
    ...
    return mapConcurrent(inputs, 8, async (input, index) => {
        ...
        if ((index + 1) % 50 === 0 || index + 1 === inputs.length) {
            process.stderr.write(`Staged ${index + 1}/${inputs.length} namecard images\n`);
        }
        return media;
    });
}
...
function reactionSignature(values) {
    return JSON.stringify([...values]
        .map((value) => ({ emoji: value.emoji, count: Number(value.count) }))
        .sort((left, right) => left.emoji.localeCompare(right.emoji)));
}
...
async function readTargetState(client, cards, reactions, mediaByCard) {
    ...
    const existingCards = new Map(cardResult.rows.map((row) => [Number(row.id), row]));
    ...
    return cards.map((card) => {
        ...
        const expectedValues = [
            media.front.url, media.back.url, card.hash1, card.hash2,
            card.ip, card.status, card.createdAt
        ];
        const existingValues = existing ? [
            existing.image1_url, existing.image2_url, existing.hash1, existing.hash2,
            existing.ip, existing.status, new Date(existing.created_at).toISOString()
        ] : null;
        return {
            id: card.id,
            rowStatus: !existing
                ? 'missing'
                : JSON.stringify(existingValues) === JSON.stringify(expectedValues)
                    ? 'unchanged'
                    : 'would-update',
            reactionStatus: reactionSignature(existingReactions.get(card.id) || []) ===
...
async function syncObjects(storage, media, apply, objectKeyForFilename) {
    return mapConcurrent(media, 4, async (entry, index) => {
        ...
        const matches = existing !== null && existing.size === entry.bytes &&
            digest('sha256', existing.body) === entry.sha256;
        let objectStatus;
        if (matches) objectStatus = 'unchanged';
        else if (!apply) objectStatus = existing ? 'would-replace' : 'would-upload';
        else {
            ...
            if (!stored || stored.size !== entry.bytes ||
                digest('sha256', stored.body) !== entry.sha256) {
                throw new Error(`Object verification failed after writing ${key}`);
            }
        }
        if ((index + 1) % 50 === 0 || index + 1 === media.length) {
            process.stderr.write(`Checked ${index + 1}/${media.length} target objects\n`);
        }
    ...
    try {
        for (const card of cards) {
            ...
            await client.query(
                ...
                [
                    card.id, media.front.url, media.back.url, card.hash1,
                    card.hash2, card.ip, card.status, card.createdAt
                ]
            );
        }
        const ids = cards.map((card) => card.id);
        await client.query('DELETE FROM card_emojis WHERE card_id = ANY($1::bigint[])', [ids]);
        for (const card of cards) {
...
async function main() {
    ...
    if (options.apply && options.confirmSource !== options.sourceBaseUrl) {
        throw new Error(`Apply requires --confirm-source ${options.sourceBaseUrl}`);
    }
    ...
    if (parseNodeObjectStorageConfig().type !== 's3') {
        throw new Error('Legacy namecard migration requires IMS_OBJECT_STORAGE=s3');
    }

    process.stderr.write(`Reading Legacy namecards from ${options.sourceBaseUrl}\n`);
    ...
    const { namecardImageObjectKey } = require('../../src/utils/storage/business-object-keys.ts');
    const { closeNodeServices, resolveNodeServices } = require('../../src/runtime/node-services.ts');
    const client = new Client({
        connectionString: process.env.DATABASE_URL,
        connectionTimeoutMillis: Number(process.env.IMS_PG_CONNECTION_TIMEOUT_MS || 5000)
    });
    let services;
    try {
        ...
        const before = await readTargetState(client, cards, reactions, mediaByCard);
        const syncedMedia = await syncObjects(
            services.storage, media, options.apply, namecardImageObjectKey
        );
        if (options.apply) {
            await reconcileDatabase(client, cards, reactions, mediaByCard);
            const verified = await readTargetState(client, cards, reactions, mediaByCard);
            if (verified.some((entry) =>
                entry.rowStatus !== 'unchanged' || entry.reactionStatus !== 'unchanged')) {
                throw new Error('Database verification failed after Legacy namecard import');
            }
        }
        ...
        const report = {
            ...
            summary: {
                ...
                mediaBytes: syncedMedia.reduce((total, entry) => total + entry.bytes, 0),
                reactionRows,
                reactionCount: [...reactions.values()].flat().reduce(
