...
interface PublicObjectPlacementServices {
...
export function parsePublicObjectPlacementArguments(
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (argument === "--apply") {
            ...
            report = path.join(
                projectRoot,
                "data/migration/public-object-placement.json",
            );
...
async function closeServices(
    services: PublicObjectPlacementServices,
): Promise<void> {
...
async function reconcile(
    ...
    const reconcilePlacement = services.storage.reconcilePlacement?.bind(
        services.storage,
    );
    if (!reconcilePlacement) {
        throw new Error(
            "The active object storage does not support placement reconciliation",
        );
    }
    const keys = [
        ...new Set(
            ...
            async (): Promise<void> => {
                while (next < keys.length) {
                    ...
                    if (!current) {
                        ...
                        const moved = await reconcilePlacement(key);
                        const verified =
                            await services.storage.createReadUrl?.(key);
                        if (!moved || verified?.visibility !== "public") {
                            throw new Error(
                                `Failed to verify public placement: ${key}`,
                            );
                        }
...
async function main(): Promise<void> {
    ...
    const services =
        (await createNodeServices()) as unknown as PublicObjectPlacementServices;
    try {
        ...
        const counts = Object.fromEntries(
            ["already-public", "missing", "moved", "would-move"].map(
                (result) => [
                    result,
        ...
        console.log(
            `Public object placement: ${entries.length} ready object(s)`,
        );
        console.log(`Report: ${options.report}`);
        if (entries.some((entry) => entry.result === "missing"))
            process.exitCode = 1;
    } finally {
