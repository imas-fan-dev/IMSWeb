...
function parseArguments(argv, environment = process.env) {
    ...
    const defaultStaging = path.join(projectRoot, 'data/migration/legacy-producer-map');
    const options = {
...
async function fetchWithRetry(url, label) {
    ...
    throw new Error(`${label} failed after 4 attempts: ${lastError?.message || 'unknown error'}`);
}
...
function sourcePath(value, directory, label) {
    ...
    if (parsed.origin !== 'https://legacy.invalid' || parsed.search || parsed.hash ||
        !parsed.pathname.startsWith(directory) || parsed.pathname === directory) {
...
async function parseLegacyPage(html) {
    ...
    const subtitles = descendants(document, (node) =>
        node.tagName === 'p' && nodeText(node).includes('THE IDOLM@STER'));
    const cards = descendants(document, (node) => {
        if (node.tagName !== 'a') return false;
        return (attributes(node).get('class') || '').split(/\s+/).includes('infonews-card');
    });
    ...
    if (!title || !subtitle || !cards.length || cards.length > 100) {
        throw new Error('Legacy Producer Map page has an invalid content shape');
    }
    ...
    if (communities.some((item) => !item.name) ||
        new Set(communities.map((item) => item.name)).size !== communities.length) {
        throw new Error('Legacy Producer Map communities are empty or duplicated');
    }
...
function parseLegacyMapScript(script, provinces) {
    ...
    for (const match of script.slice(start + 1, end).matchAll(pattern)) {
        ...
        const stem = path.posix.basename(imagePath, path.posix.extname(imagePath));
        if (!/^[a-z0-9]+$/.test(stem)) {
            throw new Error(`Legacy region ${province} has an invalid filename`);
        }
        entries.push({
    ...
    if (entries.length !== provinces.length ||
...
async function readLegacySource(sourceBaseUrl, provinces) {
    ...
    return {
        sourceBaseUrl,
        page: { url: pageUrl.toString(), bytes: page.body.byteLength, sha256: digest(page.body) },
        script: {
...
function mediaFilename(kind, item, extension) {
    ...
    if (!['region', 'community'].includes(kind) || !/^[a-z0-9-]+$/.test(stem) ||
...
async function stageSourceMedia(source, staging, producerMapAssetObjectKey) {
    ...
    return mapConcurrent(inputs, 8, async ({ kind, item }, index) => {
        ...
        const response = await fetchWithRetry(sourceUrl, `Legacy ${kind} ${item.id}`);
        if (!response.ok) {
            throw new Error(`Legacy ${kind} ${item.id} returned HTTP ${response.status}`);
        }
        const body = Buffer.from(await response.arrayBuffer());
        ...
        if ((index + 1) % 10 === 0 || index + 1 === inputs.length) {
            process.stderr.write(`Staged ${index + 1}/${inputs.length} Producer Map images\n`);
        }
        return {
            kind,
            id: item.id,
...
function nextProducerMapContent(current, source, media) {
    ...
    for (const sourceRegion of source.regions) {
        ...
        if (!asset) throw new Error(`Producer Map media is missing region ${sourceRegion.id}`);
        const index = regions.findIndex((item) => item.province === sourceRegion.province);
        if (index < 0) {
            regions.push({
                id: sourceRegion.id,
    ...
    for (const sourceCommunity of source.communities) {
        ...
        if (!asset) {
            throw new Error(`Producer Map media is missing community ${sourceCommunity.id}`);
        }
        const index = communities.findIndex((item) => item.name === sourceCommunity.name);
        if (index < 0) {
            communities.push({
...
async function syncObjects(storage, media, apply) {
    return mapConcurrent(media, 4, async (entry) => {
        ...
        if (!matches && !apply) objectStatus = existing ? 'would-replace' : 'would-upload';
        else if (!matches) {
        ...
        if (apply || matches) {
            ...
            if (!verified || verified.size !== entry.bytes ||
                digest(verified.body) !== entry.sha256) {
                throw new Error(`Producer Map object verification failed: ${entry.key}`);
            }
        }
...
async function syncProducerMapData(storage, source, media, apply, dependencies) {
    ...
    if (apply && plan.changed) {
        if (!storage.putIfUnchanged) {
            throw new Error('Producer Map migration requires conditional object writes');
        }
        ...
        if (!stored) throw new Error('Producer Map config changed during migration; retry');
        configStatus = currentObject ? 'updated' : 'created';
    }
    if (apply || !plan.changed) {
        const verified = await storage.get(dependencies.objectKey);
        if (!verified) throw new Error('Producer Map config verification failed');
        const content = dependencies.parseContent(verified.body);
        for (const entry of media) {
            const sourceItem = entry.kind === 'region'
                ? source.regions.find((candidate) => candidate.sourcePath === entry.sourcePath)
                : source.communities.find((candidate) => candidate.sourcePath === entry.sourcePath);
            const item = entry.kind === 'region'
                ? content.regions.find((candidate) => candidate.province === sourceItem?.province)
                : content.communities.find((candidate) => candidate.name === sourceItem?.name);
            if (!item) throw new Error(`Producer Map config is missing ${entry.kind} ${entry.id}`);
        }
    }
    ...
    try {
        ...
        const report = {
            ...
            summary: {
                ...
                mediaBytes: result.media.reduce((total, item) => total + item.bytes, 0),
