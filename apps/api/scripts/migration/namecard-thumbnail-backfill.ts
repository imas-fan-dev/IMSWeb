...
import {
    closeNodeServices,
    createNodeServices,
} from "../../src/runtime/node-services";
import { namecardMediaObjectKeys } from "../../src/utils/storage/business-object-keys";
...
async function main(): Promise<void> {
    ...
    try {
        ...
        for (let offset = 0; offset < total; offset += pageSize) {
            const cards = await services.namecards.listApprovedCards(
                pageSize,
                offset,
            );
            for (const card of cards) {
                ...
                if (!image1Url || !image2Url) {
                    throw new Error(
                        `Namecard ${cardId} has an invalid media URL`,
                    );
                }
                for (const side of ["front", "back"] as const) {
                    const originalUrl =
                        side === "front" ? image1Url : image2Url;
                    sides.push({
                        cardId,
                        originalUrl,
        ...
        const entries = new Array<ThumbnailEntry | null>(sides.length).fill(
            null,
        );
        let next = 0;
        let handled = 0;
            ...
            Array.from(
                ...
                async (): Promise<void> => {
                    while (next < sides.length) {
                        ...
                        try {
                            if (
                                !(await services.namecards.findApprovedCardMedia(
                                    side.cardId,
                                ))
                            ) {
                                entry.result = "not-approved-anymore";
                            } else if (
                                await services.storage.exists(side.key)
                            ) {
                                entry.result = "exists";
                            } else {
                                ...
                                if (!original) {
                                    ...
                                    const thumbnail =
                                        await services.images.resizeJpeg(
                                            original.body,
                                            NAMECARD_THUMBNAIL_WIDTH,
                                            NAMECARD_THUMBNAIL_HEIGHT,
                                            {
                                    ...
                                    if (!options.apply) {
                                        ...
                                        services.storage.putIfUnchanged
                                    ) {
                                        const stored =
                                            await services.storage.putIfUnchanged(
                                                side.key,
                                                null,
                                                thumbnail,
                                        ...
                                        if (!stored) {
                                            ...
                                            if (
                                                ...
                                                sha256(verified.body) !==
                                                    entry.sha256
                                            ) {
                                                throw new Error(
                                                    `Object verification failed after writing ${side.key}`,
                                                );
                                            }
        ...
        const report = {
            ...
            thumbnail: {
                ...
                contentType: "image/jpeg",
                publicPathPattern:
                    "/uploads/namecard/thumbnail/<original-filename>.jpg",
            },
            approvedCards: total,
            counts,
            entries: finished,
