...
function writeJsonIfChanged(filename, value) {
    if (fs.existsSync(filename)) {
        const existing = readJsonArtifact(
            filename,
...
function sourceFile(sourceRoot, exportPath) {
    ...
    const filename = path.resolve(
        root,
        ...safeRelativePath(exportPath, "inventory exportPath").split("/"),
    );
    ...
    if (
        ...
        throw new Error(
            `Inventory export path escapes source root: ${exportPath}`,
        );
    }
    return filename;
}
...
function filesUnder(root) {
    ...
    function visit(directory, segments) {
        for (const entry of fs.readdirSync(directory, {
            ...
            if (stat.isSymbolicLink()) {
                throw new Error(
                    `Fudaba media export contains a symlink: ${relative}`,
                );
            }
            if (stat.isDirectory()) visit(filename, [...segments, entry.name]);
                ...
                throw new Error(
                    `Fudaba media export contains a non-regular entry: ${relative}`,
                );
        }
    }
    ...
    if (!stat.isDirectory() || stat.isSymbolicLink()) {
        throw new Error(
            "Fudaba media source root must be a non-symlink directory",
        );
    }
    visit(root, []);
...
function normalizeContentType(value) {
    ...
    if (!ALLOWED_IMAGE_TYPES.has(contentType)) {
        throw new Error(
            `Unsupported Fudaba image content type: ${String(value)}`,
        );
    }
    return contentType;
}
...
function normalizeEtag(value) {
    const etag = String(value || "")
        .trim()
        .replace(/^"|"$/g, "");
    if (!etag || /[\x00-\x1f\x7f]/.test(etag))
        throw new Error("Inventory ETag is invalid");
    return etag;
}
...
function normalizedMetadata(value) {
    ...
    for (const [key, raw] of Object.entries(value)) {
        ...
        if (
            !normalized ||
            Object.hasOwn(output, normalized) ||
            typeof raw !== "string"
        ) {
...
function verifiedFile(filename, entry) {
    ...
    if (!before.isFile() || before.isSymbolicLink()) {
        throw new Error(
            `Inventory source is not a regular file: ${entry.exportPath}`,
        );
    }
    ...
    if (
        ...
        BigInt(body.byteLength) !== after.size
    ) {
        throw new Error(
            `Inventory source changed while reading: ${entry.exportPath}`,
        );
    }
    if (body.byteLength !== entry.bytes || sha256(body) !== entry.sha256) {
        throw new Error(
            `Inventory source bytes or SHA-256 differ: ${entry.exportPath}`,
        );
    }
...
function validateInventoryIdentity(document, snapshot) {
    if (
        ...
        document.objectCount !== document.entries.length
    ) {
        throw new Error(
            "Fudaba R2 inventory identity is incomplete or mismatched",
        );
    }
...
function loadVerifiedInventory(filename, sourceRoot, snapshot) {
    ...
    for (const raw of document.entries) {
        ...
        if (previousKey !== null && compareUtf8(previousKey, key) >= 0) {
            throw new Error(
                "Fudaba R2 inventory entries must use unique UTF-8 byte order",
            );
        }
        previousKey = key;
        ...
        const exportPath = safeRelativePath(
            raw.exportPath,
            "inventory exportPath",
        );
        ...
        const entry = {
            key,
            versionId:
                raw.versionId === null ? null : String(raw.versionId || ""),
    ...
    if (JSON.stringify(actualFiles) !== JSON.stringify(expectedFiles)) {
        throw new Error(
            "Fudaba media export files do not exactly match the frozen inventory",
        );
    }
...
function collectMediaReferences(snapshot) {
    ...
    for (const table of ["oauth_accounts", "email_credentials"]) {
        for (const row of snapshot.rows[table]) {
            if (includedDescriptor(descriptors, table, row))
    ...
    const includedSeries = new Set(
        snapshot.rows.series_tags
            .filter((row) =>
                includedDescriptor(descriptors, "series_tags", row),
            )
            .map((row) => row.name),
    );
    const references = [];
    for (const row of snapshot.rows.users) {
        ...
        if (
            !descriptor ||
            descriptor.classification === "owner-approved-reference" ||
            !includedUsers.has(row.id) ||
            row.avatar_url === ""
        )
            continue;
        references.push({
            ...
            ownerId: row.id,
            slot: "avatar",
            sourceReference: row.avatar_url,
            required: false,
            allowExternal: true,
        });
    }
    for (const row of snapshot.rows.offices) {
        if (
            !includedDescriptor(descriptors, "offices", row) ||
            !includedUsers.has(row.owner_id) ||
            row.cover_image === ""
        )
            continue;
        references.push({
            ...
            slot: "cover",
            sourceReference: row.cover_image,
            required: false,
            allowExternal: false,
        });
    }
    for (const row of snapshot.rows.cards) {
        if (
            !includedDescriptor(descriptors, "cards", row) ||
            !includedUsers.has(row.owner_id) ||
            !includedSeries.has(row.series)
        )
            continue;
...
function assertUploadedKey(reference, key) {
    ...
    if (reference.entityKind === "account") {
        ...
        filename = segments[2];
        if (
            !UUID_V4_PATTERN.test(filename.split("-avatar.")[0]) ||
        ...
        filename = segments[2];
        if (
            !UUID_V4_PATTERN.test(filename.split("-cover.")[0]) ||
        ...
        const hasAllowedExtension = ["avif", "jpg", "png", "webp"].some(
            (extension) => filename.endsWith(`${marker}${extension}`),
        );
        if (
            !UUID_V4_PATTERN.test(filename.split(marker)[0]) ||
            !hasAllowedExtension
        ) {
...
function assertSourceMetadata(reference, entry) {
    ...
    if (reference.entityKind === "office") {
        if (
            metadata.officeid !== reference.entityId ||
...
function objectKeyFor(reference, extension, builders) {
    if (reference.entityKind === "account") {
        return builders.fudabaAccountAvatarObjectKey(
            reference.entityId,
            extension,
        );
    }
    if (reference.entityKind === "office") {
        return builders.fudabaOfficeCoverObjectKey(
            reference.entityId,
            extension,
        );
    }
...
async function buildMediaPlan(snapshot, inventory, imageProcessor, builders) {
    ...
    for (const reference of collectMediaReferences(snapshot)) {
        let planEntry = {
            ...
            sourceObject: null,
            image: null,
            blocker: reference.required
                ? "required-media-is-not-a-frozen-r2-object"
                : null,
        };
        if (
            /^https:\/\//.test(reference.sourceReference) &&
            reference.allowExternal
        ) {
            let external;
            try {
                ...
                throw new Error(
                    `Invalid external avatar URL: ${reference.sourceReference}`,
                );
            }
            if (
                external.username ||
                ...
                throw new Error(
                    `Unsafe external avatar URL: ${reference.sourceReference}`,
                );
            }
            planEntry = {
                ...planEntry,
            ...
            if (!source)
                throw new Error(
                    `R2 inventory is missing referenced object: ${key}`,
                );
            if (source.bytes > MAX_IMAGE_BYTES) {
                throw new Error(
                    `Referenced Fudaba image exceeds 8 MiB: ${key}`,
                );
            }
            assertSourceMetadata(reference, source);
            const image = await imageProcessor.validate(
                source.body,
                source.contentType,
            );
            const extension = canonicalExtension(image);
            const expectedSourceExtension = path.posix
                .extname(key)
                .slice(1)
                .toLowerCase();
            if (expectedSourceExtension !== extension) {
                throw new Error(
                    `R2 key extension differs from decoded image: ${key}`,
                );
            }
            const logicalObjectKey = objectKeyFor(
                reference,
                extension,
                builders,
            );
    ...
    for (const entry of entries) {
        ...
        if (identities.has(identity))
            throw new Error(`Duplicate media plan identity: ${identity}`);
        identities.add(identity);
        if (entry.logicalObjectKey) {
            if (objectKeys.has(entry.logicalObjectKey)) {
                throw new Error(
                    `Duplicate Fudaba target object key: ${entry.logicalObjectKey}`,
                );
            }
...
function validateManifestIdentity(
    ...
    if (
        ...
        manifest.mediaPlanSha256 !== mediaPlanSha256 ||
        !Array.isArray(manifest[property])
    ) {
...
function exactEntryIndex(entries, label) {
    ...
    for (const entry of entries) {
        ...
        if (!entry || typeof entry !== "object" || index.has(identity)) {
            throw new Error(
                `${label} has an invalid or duplicate entry: ${identity}`,
            );
        }
...
function validateRights(rights, snapshot, plan, mediaPlanSha256) {
    ...
    for (const planned of plan.entries) {
        ...
        if (
            !approval ||
            approval.bindingSha256 !== planned.bindingSha256 ||
            ...
            approval.contentType !==
                (planned.sourceObject?.contentType || null) ||
            !REVIEW_STATUSES.has(approval.status)
        ) {
            throw new Error(
                `Rights approval does not match its media binding: ${identity}`,
            );
        }
        ...
        if (
            ...
            throw new Error(
                `Rights review evidence is incomplete: ${identity}`,
            );
        }
        parseTimestamp(approval.reviewedAt, "rights reviewedAt");
        if (approval.status === "approved") {
            const expected =
                planned.sourceType === "r2"
                    ? "store-protected"
            ...
            if (!expected || approval.action !== expected) {
                blockers.push({
                    identity,
                    code: "approved-action-not-supported",
                });
            }
        } else if (approval.action !== "omit" || planned.required) {
            blockers.push({
                identity,
                code: planned.required
    ...
    for (const identity of approvals.keys()) {
        if (!plannedIdentities.has(identity)) {
            throw new Error(
                `Rights manifest has an orphan approval: ${identity}`,
            );
        }
    }
...
function validateExistingMedia(media, snapshot, plan, mediaPlanSha256) {
    ...
    for (const planned of plan.entries) {
        ...
        if (
            ...
            throw new Error(
                `Media manifest entry differs from the plan: ${mediaIdentity(planned)}`,
            );
        }
    }
    if (entries.size !== plan.entries.length) {
        throw new Error(
            "Media manifest contains entries not present in the media plan",
        );
    }
    return entries;
}
...
function targetMatches(entry, target, stored) {
    return (
        ...
        stored &&
        target.state === "ready" &&
        target.storageScope === "private" &&
        typeof target.objectId === "string" &&
        target.objectId &&
        typeof target.physicalObjectKey === "string" &&
        target.physicalObjectKey &&
        typeof target.etag === "string" &&
        target.etag &&
...
async function inspectTransferTarget(entry, targetRuntime) {
    ...
    const stored = target
        ? await targetRuntime.storage.get(entry.logicalObjectKey)
        : null;
    if (!target && !stored) return { state: "missing", target: null };
    if (targetMatches(entry, target, stored))
        return { state: "unchanged", target };
    return {
        state: "conflict",
        target: target || null,
        reason: !target
            ? "unmanaged-object-visible"
            : target.storageScope !== "private"
              ? "target-is-public"
              : target.state !== "ready"
...
function finalNonTransferEntry(planned, approval) {
    ...
    if (
        ...
        approval.action === "retain-external"
    ) {
        return {
            ...base,
            state: "external",
...
function confirmationPairs(options, artifacts, sourceBucket, targetBucket) {
    return [
        ...
        [
            "--confirm-rows-sha256",
            options.confirmRowsSha256,
            artifacts.rowsSha256,
        ],
        [
        ...
        [
            ...
            artifacts.planSha256,
        ],
        [
            "--confirm-rights-sha256",
            options.confirmRightsSha256,
            artifacts.rightsSha256,
        ],
        [
            "--confirm-media-sha256",
            options.confirmMediaSha256,
            artifacts.mediaSha256,
        ],
...
function assertApplyConfirmations(
    options,
    artifacts,
    sourceBucket,
    targetBucket,
...
async function applyMissingTransfers(
    ...
    if (
        typeof runtime.storage.putIfUnchanged !== "function" ||
    ...
    try {
        for (const entry of missing) {
            ...
            if (!stored)
                throw new Error(
                    `Concurrent target mutation: ${entry.logicalObjectKey}`,
                );
            ...
            if (
                !targetMatches(entry, target, readback) ||
                target.ownerToken !== ownerToken
            ) {
                throw new Error(
                    `Protected target readback failed: ${entry.logicalObjectKey}`,
                );
            }
            created.push({
                key: entry.logicalObjectKey,
                objectId: target.objectId,
            });
            results.set(
                mediaIdentity(entry),
                finalTransferredEntry(entry, target, targetBucket),
            );
            attempted = null;
        }
...
async function runFudabaMediaMigration(options, dependencies = {}) {
    ...
    const snapshot =
        dependencies.snapshot || (await loadSnapshot(snapshotDirectory));
    ...
    const imageProcessor =
        ...
        new (require("../../src/infra/media/sharp/image-processor.ts").SharpImageProcessor)();
    const builders =
    ...
    if (
        Array.isArray(rights.approvals) &&
        rights.approvals.length === 0 &&
    ...
    if (options.apply) {
        assertApplyConfirmations(
            options,
            artifacts,
            snapshot.sourceJson.source.r2Bucket,
            targetBucket,
        );
    }
    const report = {
        ...
        summary: {
            ...
            transferEntries: plan.entries.filter(
                (entry) => entry.sourceType === "r2",
            ).length,
            externalEntries: plan.entries.filter(
                (entry) => entry.sourceType === "external",
            ).length,
            omittedEntries: 0,
            unchanged: 0,
            missing: 0,
    ...
    const reportFile = path.resolve(
        options.report ||
            path.join(snapshotDirectory, "media-reconciliation.json"),
    );
    ...
    const runtime =
        dependencies.targetRuntime || (await dependencies.resolveTarget?.());
    if (
        !runtime ||
        !runtime.storage ||
        typeof runtime.inspectTarget !== "function"
    ) {
        throw new Error("Fudaba media target runtime is unavailable");
    }
    try {
        const approvals = rightsValidation.approvals;
        const transferEntries = plan.entries
            .filter((entry) => {
        ...
        const ownerToken =
            `fudaba-media:${snapshot.sourceJson.snapshotId}:` +
            crypto.randomUUID();
        let applied;
        try {
            ...
            report.blockers.push({
                code: "target-write-failed",
                detail: error.message,
            });
            report.summary.blockers = report.blockers.length;
            writeJsonAtomic(reportFile, report);
            throw new FudabaMediaBlockedError(
                `Fudaba media apply failed: ${error.message}`,
                report,
            );
        }
        ...
        const finalEntries = plan.entries.map((entry) => {
            const identity = mediaIdentity(entry);
            const transferred =
                applied.results.get(identity) || existingResults.get(identity);
            return (
                transferred ||
                finalNonTransferEntry(entry, approvals.get(identity))
            );
        });
        const finalMedia = {
            ...media,
...
async function mediaMain(
    ...
    try {
        ...
        if (storage.type !== "s3")
            throw new Error("Fudaba media migration requires S3 storage");
