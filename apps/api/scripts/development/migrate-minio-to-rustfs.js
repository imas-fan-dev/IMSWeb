...
const BUCKET_ROOT =
  "/var/lib/docker/volumes/imsweb-minio_minio-data/_data/imsweb-media-local";
...
let stdout;
try {
  stdout = execSync("wsl find " + BUCKET_ROOT + " -name part.1 -type f", {
    encoding: "utf8",
    maxBuffer: 10 * 1024 * 1024,
  });
} catch (error) {
  throw new Error(`Failed to scan the MinIO volume at ${BUCKET_ROOT}`, {
    cause: error,
  });
}
...
(async () => {
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      try {
        await s3.send(
          new HeadObjectCommand({ Bucket: "imsweb-media-local", Key: key }),
        );
        skipped++;
        continue;
      } catch {
      ...
      const data = execSync('wsl cat "' + filePath + '"', {
