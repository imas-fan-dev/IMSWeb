...
function readComposeConfig() {
    ...
    try {
        ...
        throw new Error("Docker Compose returned invalid JSON configuration", {
            cause: error,
        });
    }
    const bucket =
        config.services?.["rustfs-init"]?.environment?.IMS_RUSTFS_BUCKET;
    if (!bucket) {
...
function startDataServices() {
    const running = new Set(
        captureCompose(["ps", "--status", "running", "--services"], {
            label: "checking development data containers",
        }).split("\n"),
    );
    const services = [];
    ...
    if (services.length === 0) {
        console.log(
            "PostgreSQL and RustFS development containers are running.",
        );
        return;
    }

    console.log(
        `Starting missing development containers: ${services.join(", ")}...`,
    );
...
function rustfsRunArguments(shellSource, mount) {
    ...
    args.push("--entrypoint", "/bin/sh", "rustfs-init", "-ec", shellSource);
    return args;
}
...
function rustfsUsage() {
    const source = captureRustfs(
        [RUSTFS_ALIAS, 'mc du --json "development/$IMS_RUSTFS_BUCKET"'].join(
            "\n",
        ),
    ...
    try {
        ...
        throw new Error("RustFS returned invalid JSON bucket usage data", {
            cause: error,
        });
    }
    if (
...
function validatePostgresDump(dumpPath) {
    ...
    try {
        runCompose(["exec", "-T", "postgres", "pg_restore", "--list"], {
    ...
    const visit = (current) => {
        for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
            ...
            if (entry.isSymbolicLink()) {
                throw new Error(
                    `Snapshot contains a symbolic link: ${candidate}`,
                );
            }
            ...
            if (!entry.isFile()) {
                throw new Error(
                    `Snapshot contains an unsupported entry: ${candidate}`,
                );
            }
...
function validateArchiveEntryTypes(entries) {
    for (const entry of entries) {
        ...
        if (type !== "-" && type !== "d") {
            throw new Error(
                "Snapshot archive may contain only files and directories",
            );
        }
    }
}
...
function validateManifest(manifest) {
    if (
        ![FORMAT_VERSION, LEGACY_FORMAT_VERSION].includes(
    ...
    if (
        manifest.archiveRoot !== ARCHIVE_ROOT ||
...
async function exportSnapshot(options) {
    ...
    try {
        ...
        if (
            rustfsBefore.objects !== rustfsAfter.objects ||
        ...
        const manifest = {
            ...
            consistency:
                "PostgreSQL and RustFS component snapshots are not atomic together",
            postgresql: {
        ...
        runProcess(
            "tar",
            ["-czf", partialArchive, "-C", workingDirectory, ARCHIVE_ROOT],
            {
...
async function restoreSnapshot(options) {
    ...
    try {
        ...
        const rustfsDirectory = path.join(snapshotDirectory, storage.directory);
        validatePostgresDumpFile(dumpPath);
        if (fs.statSync(dumpPath).size !== manifest.postgresql.bytes) {
            throw new Error(
                "Extracted PostgreSQL dump does not match the manifest",
            );
        }
        const localRustfs = summarizeDirectory(rustfsDirectory);
        if (
            localRustfs.objects !== storage.objects ||
            localRustfs.bytes !== storage.bytes
        ) {
            throw new Error(
                "Extracted object data does not match the manifest",
            );
        }
        ...
        if (
            restoredRustfs.objects !== storage.objects ||
            restoredRustfs.bytes !== storage.bytes
        ) {
            throw new Error(
                "Restored RustFS usage does not match the manifest",
            );
        }
        console.log(
