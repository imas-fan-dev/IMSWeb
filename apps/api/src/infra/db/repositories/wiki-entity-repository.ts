...
export class SqlWikiEntityRepository {
    ...
    async listThemeColors(): Promise<Record<string, string>> {
        const rows = await queryAll<{ name: string; color: string }>(
            this.database,
    ...
    async listAgencies(): Promise<AgencyRecord[]> {
        const rows = await queryAll<AgencyRecord>(
            this.database,
    ...
    async listIdolsWithAgencies(): Promise<IdolWithAgencyRecord[]> {
        const rows = await queryAll<IdolWithAgencyRecord>(
            this.database,
    ...
    async listWikiGroups(agencyId?: number): Promise<WikiGroupRecord[]> {
        const rows = await queryAll<WikiGroupRecord>(
            this.database,
    ...
    async findWikiGroupById(id: number): Promise<WikiGroupRecord | null> {
        const row = await queryOne<WikiGroupRecord>(
            this.database,
    ...
    listWikiGroupMembers(agencyId?: number): Promise<WikiGroupMemberRecord[]> {
        return queryAll<WikiGroupMemberRecord>(
            this.database,
    ...
    async listWikiCategories(
        ...
        const rows = await queryAll<WikiCategoryRecord>(
            this.database,
    ...
    async findAgencyByName(name: string): Promise<AgencyRecord | null> {
        const row = await queryOne<AgencyRecord>(
            this.database,
    ...
    async findIdolByAgencyAndName(
        agencyId: number,
        idolName: string,
    ): Promise<IdolRecord | null> {
        const row = await queryOne<IdolRecord>(
            this.database,
    ...
    async createWikiAgency(
        input: CreateWikiAgencyInput,
        ...
        if (!agency)
            throw new Error("Wiki agency insert did not return a record");
        return agency;
    }

    async updateWikiAgency(
        input: UpdateWikiAgencyInput,
    ): Promise<AgencyRecord> {
    ...
    async createWikiGroup(
        input: CreateWikiGroupInput,
    ): Promise<WikiGroupRecord> {
        if (!(await this.findAgencyById(input.agencyId)))
            throw httpError("企划不存在", 404);
        const duplicate = await queryOne<{ id: number }>(
            this.database,
        ...
        if (!createdId)
            throw new Error("Wiki group insert did not return an ID");
        return (await this.findWikiGroupById(createdId))!;
    }

    async updateWikiGroup(
        input: UpdateWikiGroupInput,
    ): Promise<WikiGroupRecord> {
    ...
    async deleteWikiGroup(
        ...
        // SAFETY: The delete statement returns GROUP_COLUMNS, the exact row shape consumed by groupRecord.
        const deleted = results[2]?.results[0] as unknown as
            | WikiGroupRecord
            | undefined;
    ...
    async createWikiIdol(input: CreateWikiIdolInput): Promise<IdolRecord> {
        ...
        const idol = await this.findIdolByAgencyAndName(
            input.agencyId,
            input.name,
        );
    ...
    async deleteWikiIdol(
        ...
        // SAFETY: The delete statement returns IDOL_COLUMNS, the exact row shape consumed by idolRecord.
        const deleted = results[4]?.results[0] as unknown as
            | IdolRecord
            | undefined;
        if (deleted) {
            return {
    ...
    private async assertWikiGroups(
        agencyId: number,
        groupIds: number[],
    ): Promise<void> {
        const uniqueIds = new Set(groupIds);
        if (uniqueIds.size !== groupIds.length)
            throw httpError("栏目不能重复", 400);
    ...
    private async saveEntityMedia(
        ...
        const readCurrent = () =>
            queryOne<{ object_key: string | null; revision: number }>(
                this.database,
    ...
    async ensureWikiCategory(
        ...
        await this.database.batch(statements);
        const category = await queryOne<WikiCategoryRecord>(
            this.database,
            `SELECT c.id, c.agency_id, c.name, c.storage_slug, c.background_eligible,
                    ic.display_order, ic.show_when_empty, c.revision
             FROM wiki_categories c
        ...
        if (!category)
            throw new Error("Wiki category association was not created");
    ...
    async updateWikiCategory(
        ...
        if (updated)
            return { status: "saved", category: categoryRecord(updated) };
        const current = (
            await this.listWikiCategories(input.agencyId, input.idolId)
    ...
    async deleteWikiCategoryAssociation(
        ...
        const category =
            (await this.listWikiCategories(agencyId, idolId)).find(
    ...
    async saveWikiLayout(
        input: WikiLayoutInput,
    ): Promise<WikiLayoutSaveResult> {
        ...
        if (
            input.groups.some(
                (group) => new Set(group.idolIds).size !== group.idolIds.length,
            ) ||
            requestedIdols.some((id) => !expectedIdols.has(id))
        ) {
            throw Object.assign(
                new Error("布局包含无效内容页，或同一栏目内存在重复内容页"),
                {
                    status: 400,
                },
            );
        ...
        try {
            ...
            return {
                status: "conflict",
                revision: current?.layout_revision ?? agency.layout_revision,
            };
        }
