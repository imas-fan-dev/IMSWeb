...
const precompressibleExtensions = new Set(['.css', '.html', '.js', '.json', '.svg']);
...
function acceptedEncodings(value: string | null): Array<'br' | 'gzip'> {
    ...
    for (const item of value.split(',')) {
        ...
        const parsed = qualityParameter ? Number(qualityParameter.trim().slice(2)) : 1;
        qualities.set(rawName, Number.isFinite(parsed) && parsed >= 0 && parsed <= 1 ? parsed : 0);
    }
...
export class NodeStaticAssets implements StaticAssets {
        ...
        for (const encoding of acceptedEncodings(request.headers.get('accept-encoding'))) {
            ...
            try {
                ...
                if (candidateStat.isFile() && !candidateStat.isSymbolicLink()) {
                    return { filePath: candidate, stat: candidateStat, encoding };
                }
            } catch (error) {
    ...
    async fetch(request: Request): Promise<Response> {
        ...
        try {
            representation = await this.selectRepresentation(request, filePath, stat);
        } catch {
            return new Response('Bad Request', { status: 400 });
        }
        const currentEtag = etag(representation.stat.size, representation.stat.mtimeMs);
        ...
        if (precompressibleExtensions.has(path.extname(filePath).toLowerCase())) {
        ...
        const stream = this.io.createReadStream(representation.filePath, range || undefined);
