...
import { parseUploadNamecardRequest } from "@/domains/community/namecards/request";
...
import { storeProtectedNamecardMedia } from "@/domains/community/namecards/media-assets";
...
export async function enforcePublicUploadLimit(
    ...
    return result.allowed
        ...
        : c.json(
              ...
              } satisfies NamecardRateLimitResponse,
              429,
          );
}

export async function handleUploadNamecard(
    c: Context<AppEnvironment>,
): Promise<Response> {
    ...
    try {
        ...
        if (files.some((file) => !file.contentType.startsWith("image/"))) {
            return c.json(
                { msg: "只允许上传图片文件" } satisfies NamecardMessageResponse,
                400,
            );
        }
        if (files.length !== 2) {
            return c.json(
                { msg: "必须上传2张图片" } satisfies NamecardMessageResponse,
                400,
            );
        }
        if (files.some((file) => file.body.byteLength > 3 * 1024 * 1024)) {
            return c.json(
                { msg: "文件过大" } satisfies NamecardMessageResponse,
                400,
            );
        }
        ...
        if (
            ...
            return c.json(
                { msg: "重复上传" } satisfies NamecardMessageResponse,
                409,
            );
        }
        ...
        await Promise.all(
            generated.map((key) =>
                deleteObjectWithCompensation(runtime, key).catch(
                    () => undefined,
                ),
            ),
        );
        ...
        if (status >= 500) {
            ...
            return c.json(
                { msg: "服务器错误" } satisfies NamecardMessageResponse,
                status as 500,
            );
        }
        return c.json(
            ...
            } satisfies NamecardMessageResponse,
            status as 400,
        );
    }
}
