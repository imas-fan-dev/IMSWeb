...
interface FudabaOfficeCursorEnvelope {
...
interface FudabaCardCursorEnvelope {
    version: typeof CURSOR_VERSION;
    kind: "cards";
    filters: FudabaCardFilters;
    after: FudabaCardCursor;
...
type FudabaCursorEnvelope =
    | FudabaOfficeCursorEnvelope
    | FudabaCardCursorEnvelope;
type FudabaCursorFilters = FudabaOfficeFilters | FudabaCardFilters;
...
function encodeCursor(value: FudabaCursorEnvelope): string {
...
function sameFilters(actual: unknown, expected: FudabaCursorFilters): boolean {
...
function parseSeriesCodes(parameters: URLSearchParams): string[] {
    const result = parameters
        .getAll("series")
        .map((value) => optionalText(value, "series", 40));
    if (
        result.length > MAX_SERIES_FILTERS ||
        result.some(
            (seriesCode) =>
                !seriesCode || !SERIES_CODE_PATTERN.test(seriesCode),
        ) ||
...
export function encodeFudabaOfficeCursor(
    ...
    if (
        !Number.isSafeInteger(after.visitorCount) ||
        after.visitorCount < 0 ||
...
export function decodeFudabaOfficeCursor(
    ...
    if (
        ...
        parsed.kind !== "offices" ||
        !sameFilters(parsed.filters, filters) ||
        !after ||
        !Number.isSafeInteger(after.visitorCount) ||
        Number(after.visitorCount) < 0 ||
        !printableId(after.id)
    ) {
        return null;
    }
...
export function encodeFudabaCardCursor(
    ...
    if (
        !printableId(after.id) ||
        new Date(after.createdAt).toISOString() !== after.createdAt
    ) {
...
export function decodeFudabaCardCursor(
    ...
    try {
        if (new Date(after.createdAt).toISOString() !== after.createdAt)
            return null;
    } catch {
        return null;
    }
...
export function parseFudabaOfficeQuery(url: string): FudabaOfficeQuery {
    ...
    const after = cursorValue
        ? decodeFudabaOfficeCursor(cursorValue, filters)
        : undefined;
...
export function parseFudabaCardQuery(url: string): FudabaCardQuery {
    ...
    const cursorValue = parameters.get("cursor");
    const after = cursorValue
        ? decodeFudabaCardCursor(cursorValue, filters)
        : undefined;
...
function decimal(value: string, name: string): number {
    if (!DECIMAL_PATTERN.test(value))
        throw badRequest(`bbox ${name} is invalid`);
...
function scaledBoundary(value: string, direction: "lower" | "upper"): number {
    ...
    if (!negative) {
        return Number(
            direction === "lower" && hasRemainder ? tenths + 1n : tenths,
        );
    }
    return Number(
        direction === "upper" && hasRemainder ? -(tenths + 1n) : -tenths,
    );
}
...
export function parseFudabaMapQuery(
    url: string,
