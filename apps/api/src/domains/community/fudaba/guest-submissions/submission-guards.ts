...
import { guestSubmissionWithdrawalToken } from '@/domains/community/fudaba/guest-submissions/request';
import type { GuestSubmissionRateLimitResponse } from '@/domains/community/fudaba/guest-submissions/response';
...
export async function enforceSubmissionLimit(
    ...
    return byIp.allowed && bySubmission.allowed
        ? null
        : c.json({ error: 'Too many requests' } satisfies GuestSubmissionRateLimitResponse, 429);
}

export async function withdrawalTokenHash(
    c: Context<AppEnvironment>,
): Promise<string | null> {
    const token = guestSubmissionWithdrawalToken(c.req.raw);
    return token
