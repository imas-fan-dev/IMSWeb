...
const categoryFolders: Record<string, string> = {
  ...
  enzaP卡: "enza_pcard",
  enzaS卡: "enza_scard",
  scspP卡_电话: "scsp_pcard_call",
  scspS卡: "scsp_scard",
  "【W.I.N.G.編】": "wing",
  "【ファン感謝祭編】": "fan_festival",
  "【G.R.A.D.編】": "grad",
  "【Landing Point編】": "landing_point",
  "【S.T.E.P.編】": "step",
  "ytb，X链接 [需要VPN]": "ytb_x_link",
  b站的一些切片: "bilibili_clips",
  "🍫情人节": "valentine",
  "🍬白色情人节": "whiteday",
  "🃏愚人节": "april_fools",
  "🌟闪耀日": "shiny_day",
...
export function isSupportedAgencyCode(
  code: string,
): code is SupportedAgencyCode {
...
export function wikiStaticObjectKey(
  namespace: string,
  relativePath: string,
): string {
  if (!["icon", "css", "assets"].includes(namespace))
    throw new Error("invalid Wiki static namespace");
...
export function agencyIconObjectKey(code: string): string {
  if (!isSupportedAgencyCode(code)) throw new Error("invalid Wiki agency code");
  return wikiStaticObjectKey("icon", `agencies/${code}.webp`);
}

export function agencyIconUrl(code: string, etag?: string): string {
  if (!isSupportedAgencyCode(code)) throw new Error("invalid Wiki agency code");
  const path = `/icon/agencies/${encodeURIComponent(code)}.webp`;
  if (!etag) return path;
  const version = etag.replace(/^W\//, "").replaceAll('"', "");
  return `${path}?v=${encodeURIComponent(version || etag)}`;
}

export async function listAgencyIconUrls(
  storage: ObjectStorage,
): Promise<Map<SupportedAgencyCode, string>> {
  const prefix = `${wikiStaticObjectKey("icon", "agencies")}/`;
  const objects = new Map(
    (await storage.list(prefix)).map((object) => [object.key, object]),
  );
  const result = new Map<SupportedAgencyCode, string>();
  for (const code of SUPPORTED_AGENCY_CODES) {
    const object = objects.get(agencyIconObjectKey(code));
    if (object) result.set(code, agencyIconUrl(code, object.etag));
  }
  return result;
}
...
export async function findAvatar(
  ...
  for (const extension of avatarExtensions) {
    if (
      await storage.exists(storyObjectKey(code, folderName, `icon${extension}`))
    ) {
      return {
...
export async function buildHomeAgencies(
  ...
  const grouped = new Map<
    string,
    { name: string; color: string; idols: WikiIdol[] }
  >();
  ...
  return [...grouped.values()].sort(
    (left, right) =>
      (agencyOrder[left.name] ?? 99) - (agencyOrder[right.name] ?? 99),
  );
}

export async function buildIdolMediaCatalog(
  repository: StoryRepository,
  storage: ObjectStorage,
) {
...
export async function importLegacyIdolMedia(
  ...
  for (const row of rows) {
    const legacy = legacyCharacterAvatarMedia(
      row.agency_code,
      row.folder_name,
      row.name_cn,
    );
    ...
    try {
      const source = await staticAssets.fetch(
        new Request(new URL(legacy.url, requestUrl), { method: "GET" }),
      );
...
export function getPresetCategories(agency: string, idol: string): string[] {
  ...
  if (agency === "876PRO")
    return new Set(["日高爱", "水谷绘理", "秋月凉"]).has(idol)
      ? []
      : ["ytb，X链接 [需要VPN]", "b站的一些切片"];
  if (agency === "灰姑娘女孩")
    return idol === "活动剧情" ? [] : ["特殊", "卡剧情"];
  if (agency === "百万现场") {
    return new Set([
      "主线剧情",
      "活动剧情",
  ...
  if (agency === "SideM")
    return new Set(["朗读剧", "成长之星"]).has(idol)
      ? []
      : ["个人剧情", "组合剧情", "活动剧情", "卡剧情", "节日_生日", "其他"];
  if (agency === "闪耀色彩") {
    if (idol === "enza组合")
      return [
        "illumination STARS",
    ...
    if (new Set(["ルビー", "MEMちょ", "有馬かな", "黒川あかね"]).has(idol))
      return ["enza主线", "enzaP卡", "enzaS卡"];
  ...
  if (agency === "学园偶像大师")
    return new Set(["活动剧情", "主线剧情", "S卡", "根绪亚纱里"]).has(idol)
      ? []
      : ["初", "N.I.A", "STEP3", "P卡"];
...
export function aggregateStories(
  ...
  return [...categories.entries()]
    .filter(
      ([name, cards]) => presetCategories.includes(name) || cards.size > 0,
    )
  ...
  for (const [code, categories] of Object.entries({
    ...
    try {
      ...
      if (story?.image_file) {
        ...
        const idol = agency
          ? await repository.findIdolByAgencyAndName(agency.id, story.idol_name)
          : null;
        if (agency && idol) {
          const key = storyObjectKey(
            agency.code,
            idol.folder_name,
            story.image_file,
          );
