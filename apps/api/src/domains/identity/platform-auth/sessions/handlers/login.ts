...
// This fixed, invalid-account hash equalizes password verification work; it is not a credential.
const DUMMY_BCRYPT_HASH =
    // pi-lens-ignore: opengrep:generic.secrets.security.detected-bcrypt-hash.detected-bcrypt-hash
    ...
    try {
        ...
        return Boolean(
            identity.credential.salt &&
                ...
                isMigratedPbkdf2Parameters(
                    identity.credential.parameters_json,
                ) &&
...
function invalidCredentials(c: Context<AppEnvironment>): Response {
    return c.json(
        { success: false, code: "PLATFORM_CREDENTIALS_INVALID" },
        401,
    );
}
...
export async function handlePlatformLogin(
    ...
    if (identity.account.status === "suspended") {
        return c.json(
            { success: false, code: "PLATFORM_ACCOUNT_SUSPENDED" },
            403,
        );
    }
    if (identity.account.status === "deleted") {
        return c.json(
            { success: false, code: "PLATFORM_ACCOUNT_UNAVAILABLE" },
            403,
        );
    }
    if (
        ...
        if (
            !current ||
            current.account.id !== identity.account.id ||
            (!upgraded &&
                !(await credentialMatches(c, input.password, current)))
        ) {
            return invalidCredentials(c);
        }
        identity = current;
