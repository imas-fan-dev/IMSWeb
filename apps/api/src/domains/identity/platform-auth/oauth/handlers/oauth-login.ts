...
const PROVIDER_CODES: PlatformOAuthProviderCode[] = ["google", "github"];

function isProviderCode(
    value: string | undefined,
): value is PlatformOAuthProviderCode {
    return Boolean(
        value && PROVIDER_CODES.includes(value as PlatformOAuthProviderCode),
    );
}
...
export async function handlePlatformOAuthProviders(
    c: Context<AppEnvironment>,
): Promise<Response> {
    const oauth = services(c).platformOAuth;
...
export async function handlePlatformOAuthCallback(
    ...
    try {
        ...
        if (!identity) {
            const now = Date.now();
            const created = await platformAccountRepository(
                c,
            ).createOAuthAccount({
