...
export const wikiErrorBody = (msg: string): WikiErrorResponse => ({
    status: "error",
    msg,
});
...
export async function authorizeWikiRead<E extends Env>(
    ...
    try {
        const claims =
            token.source === "legacy"
                ? await (services.backofficeTokens.verifyLegacyCookie?.(
                      token.value,
...
export async function authorizeWikiWrite<E extends Env>(
    ...
    if (
        !csrf ||
        typeof claims.csrfSecret !== "string" ||
...
export async function writeWikiAudit<E extends Env>(
    ...
    if (!services.audit || !services.backofficeTokens) return;
    const token = wikiBackofficeAccessToken(context);
    if (!token) return;
    try {
        const claims =
...
export async function parseWikiUpload(
    ...
    const maxBytes =
        services.config?.storyMaxUploadBytes ?? DEFAULT_STORY_UPLOAD_MAX_BYTES;
    ...
    if (contentType.startsWith("application/x-www-form-urlencoded")) {
        ...
        if (new TextEncoder().encode(body).byteLength > maxBytes) {
            throw Object.assign(new Error("上传文件超过大小限制"), {
                status: 413,
            });
        }
        return {
    ...
    if (!services.uploads)
        throw new Error("Wiki upload parser is not configured");
...
export function optionalWikiCatalogId(
    value: unknown,
    label: string,
): number | undefined {
...
export async function resolveWikiStorySources(
    ...
    const contentTypesById = new Map(
        contentTypes.map((option) => [option.id, option]),
    );
    ...
    const defaultContentType =
        contentTypes.find(
            (option) => option.is_active && option.name === "剧情",
        ) ?? contentTypes.find((option) => option.is_active);
    const defaultSourcePlatform =
        sourcePlatforms.find(
            (option) => option.is_active && option.name === "其他来源",
    ...
    if (!defaultContentType || !defaultSourcePlatform) {
        throw Object.assign(new Error("请先在管理端启用内容类型和来源平台"), {
            status: 409,
        });
    }
    return drafts.map((draft, index) => {
        ...
        if (!contentType) {
            throw Object.assign(
                new Error(`第 ${index + 1} 个来源的内容类型不存在`),
                {
                    status: 400,
                },
            );
...
export async function findWikiMutationTarget(
    ...
    const agencyRecord =
        (await repository.findAgencyByName(agencyName)) ??
        (await repository.findAgencyByCode(agencyName));
    const agency = agencyRecord ? toWikiAgency(agencyRecord) : null;
    if (!agency) {
        return {
            error: wikiJson(wikiErrorBody("企划不存在"), errorStatus),
        } as const;
    }
    const idolRecord = await repository.findIdolByAgencyAndName(
        agency.id,
        idolName,
    );
    if (!idolRecord) {
        return {
            error: wikiJson(wikiErrorBody("找不到该内容页"), errorStatus),
        } as const;
    }
...
export async function findWikiAgencyTarget(
    ...
    const repository = services.story!;
    const agencyRecord =
        (await repository.findAgencyByName(agencyNameOrCode)) ??
...
export async function cleanupWikiObjects(
    ...
    for (const [index, result] of results.entries()) {
        if (result.status === "rejected") {
            console.error(
                "Failed to clean committed Wiki object: %s",
                uniqueKeys[index],
                result.reason,
            );
        }
...
export async function cleanupWikiObjectPrefix(
    ...
    try {
        keys.push(
            ...(await services.storage!.list(directoryPrefix))
                .filter((object) => object.key.startsWith(directoryPrefix))
                .map((object) => object.key),
        );
    } catch (error) {
        console.error(
            "Failed to enumerate committed Wiki prefix: %s",
            directoryPrefix,
            error,
        );
    }
