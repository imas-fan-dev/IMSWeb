...
export function createHandleListAdminWikiStories<E extends Env>(
  ...
  return async (context) => {
    ...
    const agencyRecord =
      (await services.story!.findAgencyByName(agencyName)) ??
    ...
    const [stories, cardRows, categoryRows, contentTypes, sourcePlatforms] =
      await Promise.all([
    ...
    const [idolImageUrl, resolvedCards, resolvedStories] = await Promise.all([
      ...
      Promise.all(
        cardRows.map(async (card) => ({
          ...
          imageUrl:
            card.cover_asset_object_key || card.image_file
              ? revisionedUrl(
      ...
      Promise.all(
        stories.map(async (story) => ({
          ...
          imageUrl:
            story.cover_asset_object_key || story.image_file
              ? revisionedUrl(
