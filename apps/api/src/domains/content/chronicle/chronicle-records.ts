...
export type ChronicleMetaDocument = ChronicleRecord[] | ChronicleMeta;
...
export function chroniclePrefix(
    bucket: string,
    activityId = "",
    filename = "",
): string {
...
export async function listChronicleObjects(
    ...
    return (await storage.list(prefix)).filter((object) =>
...
export function recordsFromChronicleMeta(meta: unknown): ChronicleRecord[] {
    ...
    if (
        meta &&
        typeof meta === "object" &&
        Array.isArray((meta as ChronicleMeta).records)
    ) {
...
export function withChronicleRecords(
    meta: unknown,
    records: ChronicleRecord[],
): ChronicleMetaDocument {
...
async function readMetaSnapshot(
    ...
    try {
        ...
        throw new Error(`编年史元数据无法解析: ${activityId}`, {
            cause: error,
        });
    }
}
...
export async function cleanupCommittedChronicleObject(
    ...
    try {
        ...
        console.error(
            "Failed to clean media for committed Chronicle mutation",
            error,
        );
    }
}
