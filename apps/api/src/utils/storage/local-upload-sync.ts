...
async function filesUnder(directory: string): Promise<string[]> {
    ...
    for (const entry of entries) {
        ...
        if (entry.isSymbolicLink()) {
            throw new Error(
                `Upload media source must not contain symlinks: ${target}`,
            );
        }
...
function logicalKey(sourceRoot: string, sourcePath: string): string {
    ...
    return publicMediaObjectKey(
        ["uploads", ...segments].join("/").normalize("NFC"),
    );
}
...
export async function listLocalUploadFiles(
    ...
    const files = (
        await Promise.all(
            BUSINESS_DIRECTORIES.map((directory) =>
...
export async function syncLocalUploads(
    ...
    for (const sourcePath of files) {
        ...
        try {
            ...
            throw new Error(
                `Failed to read object-storage key ${key} for local upload ${sourcePath}`,
                { cause: error },
            );
        }
        ...
        if (existingMatches) {
            ...
            try {
                ...
                throw new Error(
                    `Failed to write or verify local upload ${sourcePath} at object-storage key ${key}`,
                    { cause: error },
                );
            }
            if (
                ...
                sha256(stored.body) !== digest
            ) {
                throw new Error(
                    `Object-storage verification failed after writing ${key}`,
                );
            }
            verified += 1;
        }
