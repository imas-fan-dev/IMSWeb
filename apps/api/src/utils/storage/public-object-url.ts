...
export async function resolvePublicMediaUrl(
    ...
    try {
        return await resolvePublicObjectUrl(
            storage,
            publicMediaObjectKey(value),
            value,
        );
    } catch {
        return value;
...
export async function resolvePublicMediaFields<
    T extends Record<string, unknown>,
>(storage: ObjectStorage, record: T, fields: readonly string[]): Promise<T> {
