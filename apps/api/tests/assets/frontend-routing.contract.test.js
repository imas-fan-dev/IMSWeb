...
const { NodeStaticAssets } = require(
  path.join(SERVER_ROOT, "infra/http/filesystem/static-assets.js"),
);
const { resolveFrontendRoute } = require(
  path.join(SERVER_ROOT, "routing/frontend-route-policy.js"),
);
...
function walkFiles(directory, root = directory) {
  return fs.readdirSync(directory, { withFileTypes: true }).flatMap((entry) => {
    ...
    assert.equal(
      ...
      false,
      `build/client contains a symlink: ${absolute}`,
    );
    if (entry.isDirectory()) return walkFiles(absolute, root);
    assert.equal(
      entry.isFile(),
      true,
      `build/client contains a non-file: ${absolute}`,
    );
...
const app = createHonoApp(() => ({
  staticAssets: new ContractStaticAssets(frontendFiles),
}));
...
test("[FRT-02] real prerendered documents and selective SPA routes use build/client", async () => {
  for (const route of [
    ...
    await assertFileResponse(
      `/${route}`,
      path.join(FRONTEND_ROOT, route, "index.html"),
    );
    await assertFileResponse(
      `/${route}/`,
      path.join(FRONTEND_ROOT, route, "index.html"),
  ...
  for (const route of ["wiki/classic", "story/classic"]) {
    await assertFileResponse(
      `/${route}`,
      path.join(FRONTEND_ROOT, route, "index.html"),
    );
  ...
  assert.equal(
    ...
    String(fs.statSync(fallback).size),
  );
});
...
test("[FRT-05] build assets require an exact entry in the real file set", async () => {
  const javascript = frontendFileList.find(
    (file) => file.startsWith("assets/") && file.endsWith(".js"),
  );
  assert.ok(
    javascript,
    "build/client must contain a compiled JavaScript asset",
  );
  await assertFileResponse(
    `/${javascript}`,
    path.join(FRONTEND_ROOT, javascript),
  );
  ...
  assert.deepEqual(
    resolveFrontendRoute(
      { method: "GET", pathname: `/${javascript}` },
      new Set(),
    ),
    { kind: "not-found" },
  );
