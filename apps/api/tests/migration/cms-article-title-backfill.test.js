...
test.describe('CMS article', () => {
    test('title split only accepts a non-empty leading bracket title', () => {
    ...
    test.describe('body', () => {
        test('prefix preserves internal line breaks and existing nodes', () => {
        ...
        test('remains byte-for-byte unchanged when no text is moved', () => {
    ...
    test.describe('title backfill', () => {
        test('arguments are dry-run by default and validate input', () => {
        ...
        test('writes a restricted report and a content-free summary', async () => {
    ...
    test.describe('PostgreSQL', () => {
        test('dry-run, apply, and repeat preserve the migration contract', {
        ...
        test('apply rejects an invalid candidate before writing any article', {
            skip: !postgresIntegrationEnabled()
        }, async () => {
            const harness = await createPostgresTestHarness();
            onTestFinished(() => harness.close());
        ...
        test('apply reports an update conflict and rolls back earlier rows', {
            skip: !postgresIntegrationEnabled()
        }, async () => {
            const harness = await createPostgresTestHarness();
            onTestFinished(() => harness.close());
    ...
    `);
        ...
        test('apply rolls back the batch after a late event write failure', {
            skip: !postgresIntegrationEnabled()
        }, async () => {
            const harness = await createPostgresTestHarness();
            onTestFinished(() => harness.close());
