...
import { createWikiFixture, formFields, postMultipart } from "./fixture";
...
describe("Wiki admin dynamic data contract", () => {
    ...
    test("content types and source platforms are managed dynamically and protected in use", async () => {
        ...
        assert.equal(
            ((await updatePlatform.json()) as any).option.isActive,
            false,
        );
        ...
        assert.ok(
            catalogBody.contentTypes.some(
                (option: any) => option.name === "电话",
            ),
        );
        assert.ok(
            catalogBody.sourcePlatforms.some(
                (option: any) =>
    ...
    test("catalog is built from repository agencies and idols", async () => {
        ...
        assert.deepEqual(body.agencies[5], {
            ...
            layoutRevision: 0,
            iconUrl:
                "https://cdn.example.test/wiki/agencies/sc/branding/icon.webp?v=2",
    ...
    test("catalog writes create a dynamic agency and keep one idol in multiple groups", async () => {
        ...
        const createAgency = await fixture.app.request(
        ...
        const fallback = fixture.story.groups.find(
            (group) => group.agency_id === agencyId && group.is_fallback,
        );
        assert.ok(fallback);
        ...
        assert.deepEqual(
            agency.idols.find(
                (candidate: any) => candidate.id === ungroupedIdol.id,
            ).groupIds,
            [],
        );
        assert.deepEqual(
            agency.idols.find((candidate: any) => candidate.id === idol.id)
                .groupIds,
            [fallback.id, group.id],
        );
        assert.deepEqual(
            agency.groups
                .filter((candidate: any) => candidate.idolIds.includes(idol.id))
                .map((candidate: any) => candidate.id),
            [fallback.id, group.id],
        );

        const updateIdol = await fixture.app.request(
        ...
        assert.match(
            ((await invalidWikiUrl.json()) as any).msg,
            /HTTP 或 HTTPS/,
        );

        const updateGroup = await fixture.app.request(
        ...
        assert.equal(
            ((await updateGroup.json()) as any).group.name,
            "组合 A 改",
        );

        const updateAgency = await fixture.app.request(
        ...
        assert.equal(
            ((await updateAgency.json()) as any).agency.name,
            "未来企划 改",
        );
    });
    ...
    test("an empty story category can be added explicitly for one idol", async () => {
        ...
        assert.ok(
            (await fixture.story.listWikiCategories(6, 6)).some(
    ...
    test("selected idol stories expose database rows and managed category choices", async () => {
        ...
        assert.ok(
            body.categories.some(
                (category: any) => category.name === "enzaP卡",
            ),
        );
        assert.ok(
            body.categories.some(
                (category: any) => category.name === "自定义分类",
            ),
        );
        assert.ok(
            body.contentTypes.some((option: any) => option.name === "剧情"),
        );
        assert.ok(
            body.sourcePlatforms.some(
                (option: any) => option.name === "Bilibili",
            ),
        );
    ...
    test("story reads validate target parameters without leaking unrelated data", async () => {
        ...
        const missing = await fixture.app.request("/api/admin/wiki/stories", {
            headers,
        });
