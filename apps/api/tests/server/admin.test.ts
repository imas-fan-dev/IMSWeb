...
import { createTestApp, TEST_ORIGIN, testRequest } from './test-app';
...
import type { AdminRole, DeletePlatformOAuthIdentityResult, ForceLogoutPlatformAccountInput, ListPlatformAccountsForAdminInput, PlatformAccountAdminRecord, PlatformAccountAdminSearchField, PlatformOAuthLinkRecord, SetPlatformAccountStatusInput, SetPlatformAccountStatusResult } from '@/ports/repositories';
import type { RuntimeServices } from '@/ports/runtime-services';
import { adminAccountErrorResponseSchema, adminAccountListSchema, adminAccountMutationSchema, adminAuditLogListSchema, adminLogoutSuccessResponseSchema } from '@imsweb/contracts/admin';
import { adminApiPath } from '@imsweb/contracts/paths';
import { adminPlatformUserDetailResponseSchema, adminPlatformUserHttpErrorSchema, adminPlatformUserListSchema, adminPlatformUserOAuthUnlinkResponseSchema, adminPlatformUserPasswordResetResponseSchema, adminPlatformUserSessionRevocationResponseSchema, adminPlatformUserStatusResponseSchema } from '@imsweb/contracts/platform/admin-users';
...
// admin-accounts.contract.test.ts
{
    ...
    describe('admin accounts', () => {
        postgresTest('only the super administrator can list op accounts', async () => {
        ...
        postgresTest('audit logs use the shared response contract', async () => {
            const fixture = await createFixture();
            onTestFinished(() => fixture.close());
        ...
        postgresTest('super administrator creates only regular op accounts and audits the mutation', async () => {
            const fixture = await createFixture();
            onTestFinished(() => fixture.close());
        ...
        postgresTest('super administrator deletes a regular op and revokes its refresh sessions', async () => {
            const fixture = await createFixture();
            onTestFinished(() => fixture.close());
        ...
        test.describe('administrator deletion preserves', () => {
            postgresTest('resolved Fudaba moderation actors', async () => {
                const fixture = await createFixture();
                onTestFinished(() => fixture.close());
         ...
         VALUES ('retained-actor-case', 'office', 'office-1', NULL,
            ...
            postgresTest('Fudaba public-location reviewers', async () => {
                const fixture = await createFixture();
                onTestFinished(() => fixture.close());
...
// admin-platform-users.contract.test.ts
{
    ...
    test.describe('platform-user', () => {
        test('administration requires a super admin', async () => {
        ...
        test('list paginates and searches by id, email and display name', async () => {
        ...
        test('detail surfaces OAuth links and 404s for unknown ids', async () => {
            const { app, platformAccounts, tokens, csrf } = configure();
        ...
        test('request validation stays strict', async () => {
            const { app, tokens, csrf } = configure();
            const headers = await superAdminHeaders(tokens, csrf);
