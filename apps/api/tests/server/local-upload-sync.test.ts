...
test('local upload sync maps only mutable business media to stable logical keys', async () => {
...
test('local upload sync is read-only by default, verifies writes, and is idempotent', async () => {
    const directory = await fixture();
...
test('local upload sync refuses symlinks in the migration source', async () => {
    const directory = await fixture();
    await fs.symlink(
