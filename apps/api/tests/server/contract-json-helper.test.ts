...
test.describe('contract JSON helpers', () => {
    test('preserve status, media type, and untouched payload equality', async () => {
        assert.deepEqual(
    ...
    test('reject non-JSON media types and malformed JSON parameters', async () => {
    ...
    test('reject a mismatched expected status', async () => {
        await assert.rejects(
            assertContractJson(jsonResponse({ success: true, value: 'wire' }), 201, exactSchema)
        );
    });

    test('detect schema stripping against the original wire payload', async () => {
        await assert.rejects(
            readContractJson(
