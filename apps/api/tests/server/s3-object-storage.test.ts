    ...
    async send(command: unknown): Promise<any> {
        ...
        if (command instanceof ListObjectsV2Command) {
            ...
            return {
                Contents: page.map((key) => ({
                    Key: key,
                    Size: this.object(command.input.Bucket!, key)!.body
                        .byteLength,
                    ETag: this.object(command.input.Bucket!, key)!.etag,
                })),
                IsTruncated: next < keys.length,
                NextContinuationToken:
                    next < keys.length ? String(next) : undefined,
            };
        }
        if (command instanceof DeleteObjectsCommand) {
            for (const object of command.input.Delete?.Objects || []) {
                if (object.Key) {
                    this.objects.delete(
                        `${command.input.Bucket}/${object.Key}`,
                    );
                }
            }
            return {};
...
describe('S3 object storage', () => {
    test("preserves logical keys across versioned CRUD, copy, and move", async () => {
        ...
        const firstSnapshot = await state.snapshot(
            "uploads/news/original/a b.webp",
        );
        ...
        assert.deepEqual(
            (await storage.get("uploads/news/original/a b.webp"))?.body,
            firstBody,
        );
        ...
        assert.equal(
            await storage.exists("uploads/news/original/missing.webp"),
            false,
        );
        ...
        assert.equal(
            await storage.exists("uploads/news/original/copy.webp"),
            false,
        );
        assert.equal(
            await storage.exists("uploads/news/original/moved.webp"),
            true,
        );

        assert.deepEqual(
            (await storage.list("uploads/news/original/")).map(
                (object) => object.key,
            ),
            [
                "uploads/news/original/a b.webp",
    ...
    test("validates checksums and rejects unsafe logical keys", async () => {
    ...
    test("removes a write when PutObject times out after persistence", async () => {
        ...
        const cleanup = await connection
            .prepare(
        ...
        assert.equal(
            (
                await connection
                    .prepare("SELECT state FROM s3_compensation_jobs WHERE kind=?")
                    .bind("delete-s3-object")
                    .first<{ state: string }>()
            )?.state,
            "completed",
        );
    ...
    test("keeps owner tokens out of user metadata", async () => {
        const { client, state, storage } = await fixture();
        ...
        assert.equal(
            Object.keys(metadata || {}).some(
                (name) => name.toLowerCase() === "ownertoken",
            ),
            false,
        );
    });
    ...
    test("S3 physical keys support both an optional custom prefix and no prefix", () => {
        ...
        assert.equal(
            s3PhysicalObjectKey(
                ...
                "site-packages/package/source.zip",
                "object-id",
                "private",
            ),
    ...
    test("signs GET and HEAD reads without proxying object bodies", async () => {
        ...
        assert.equal(
            await storage.createReadUrl("uploads/news/original/missing.webp"),
            null,
        );
    ...
    test("S3 public and protected objects share one bucket with distinct read paths", async () => {
        ...
        assert.equal(
            client.hasObject("ims-media-prod", snapshot.physicalKey!),
            true,
        );
        ...
        await storage.put(namecardKey, new Uint8Array([4]), {
        ...
        assert.match(
            pendingNamecard.physicalKey!,
            /\/__protected\/community\/namecards\//,
        );
        assert.equal(await storage.createPublicReadUrl(namecardKey), null);
        assert.equal(
            (await storage.createReadUrl(namecardKey))?.visibility,
            "private",
        );
        ...
        assert.equal(
            client.hasObject("ims-media-prod", pendingNamecard.physicalKey!),
            false,
        );
        assert.match(
        ...
        assert.equal(
            client.hasObject("ims-media-prod", publishedPhysicalKey),
            false,
        );
        ...
        assert.equal(
            client.hasObject("ims-media-prod", readyInternal.physicalKey!),
            true,
        );
        assert.equal(
            (await storage.createReadUrl(readyInternalKey))?.visibility,
            "public",
        );
    });
    ...
    test("S3 deferred public media stays private until publication moves it", async () => {
        ...
        assert.equal(
            client.hasObject("ims-media-prod", pending.physicalKey!),
            true,
        );
        assert.match(
            pending.physicalKey!,
            /\/__protected\/editorial\/events\/assets\//,
        );
        ...
        assert.equal(
            client.hasObject("ims-media-prod", published.physicalKey!),
            true,
        );
        assert.equal(
            client.hasObject("ims-media-prod", pending.physicalKey!),
    ...
    test("S3 deferred publication hides new objects and restores the previous version on rollback", async () => {
        ...
        assert.equal(
            (await storage.list("uploads/news/original/")).some(
    ...
    test("S3 lifecycle fences owner and object identity mutations", async () => {
        ...
        assert.equal(
            await storage.moveIfOwned(key, `${key}.moved`, "owner-b"),
            false,
        );
        assert.equal(
            await storage.moveIfOwned(key, `${key}.moved`, "owner-a"),
            true,
        );
        assert.equal(await storage.exists(key), false);
        assert.deepEqual(
            (await storage.get(`${key}.moved`))?.body,
    ...
    test("S3 stale recovery and SQL compensation remove unreferenced physical versions", async () => {
        ...
        assert.ok(stale);
        await connection
            .prepare("UPDATE s3_upload_operations SET updated_at=0 WHERE id=?")
            .bind(stale.operationId)
            .run();
