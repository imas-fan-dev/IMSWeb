...
import { decodeEventCursor, encodeEventCursor } from '@/domains/content/events/event-cursor';
...
import { failureMessageResponseSchema, messageErrorResponseSchema } from '@imsweb/contracts/common';
import { createEventResponseSchema, eventErrorResponseSchema, eventLegacyPageSchema, eventListItemSchema, eventMutationResponseSchema, eventPageSchema } from '@imsweb/contracts/events';
...
// events-pagination.test.ts
{
    ...
    describe('events pagination', () => {
        postgresTest('legacy event pagination keeps its response shape and validates page and size', async () => {
        ...
        test.describe('cursor event pagination', () => {
            postgresTest('holds an id snapshot while new events are inserted', async () => {
            ...
            postgresTest('returns an explicit empty snapshot', async () => {
        ...
        postgresTest('event updates require the expected current image reference', async () => {
        ...
        postgresTest('event list responses preserve a title with legacy leading or trailing whitespace verbatim', async () => {
            const fixture = await createFixture(0);
        ...
        postgresTest('events mounted JSON routes preserve shared schemas and project query and multipart extras', async () => {
            const fixture = await createFixture(1);
        ...
        postgresTest('event cursors retain decimal BIGINT ids and reject invalid pagination modes', async () => {
