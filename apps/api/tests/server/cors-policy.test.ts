...
test.describe('CORS', () => {
    test('allows loopback development origins', async () => {
    ...
    test('allows the packaged client origins without granting credentials', async () => {
        const app = createHonoApp(() => ({}));
        for (const origin of [
            'tauri://localhost',
    ...
    test('preflight admits the bearer opt-in headers', async () => {
        const app = createHonoApp(() => ({}));
    ...
    test('does not reflect external origins', async () => {
        const app = createHonoApp(() => ({}));
        const response = await app.request('http://api.test/api/health/live', {
