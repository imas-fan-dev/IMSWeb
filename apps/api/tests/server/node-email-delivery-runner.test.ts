...
test('email delivery runner drains claims committed while shutdown begins', async () => {
...
test('email delivery runner leaves delayed claims for lease recovery after shutdown times out', async () => {
    const claimStarted = deferred();
    const releaseClaims = deferred<PlatformEmailDeliveryClaim[]>();
...
test('email delivery runner waits for graceful work and bounds stalled shutdown', async () => {
...
test('email delivery runner readiness expires and refreshes after a successful poll', async () => {
    let now = 1_000;
    const runner = new NodeEmailDeliveryRunner(
        workerStore(),
        payloadCipher,
