...
test.describe('runtime adapters', () => {
    test("Node repository initialization closes every constructed resource after partial failure", async () => {
        ...
        const core = {
            ...
            async close() {
                calls.push("core:close");
            },
        };
        const platform = {
            async initialize() {
                calls.push("platform:init");
            },
            async close() {
        ...
        const fudaba = {
            async initialize() {
                calls.push("fudaba:init");
            },
            async close() {
                calls.push("fudaba:close");
            },
        };
        const story = {
            async initialize() {
                calls.push("story:init");
                throw new Error("story init failed");
            },
            async close() {
                calls.push("story:close");
            },
        };
    ...
    test("story upload byte limit accepts only a positive bounded safe integer", () => {
        ...
        for (const value of ["", "abc", "0", "-1", "1.5", "Infinity", "52428801"]) {
            assert.throws(
                () => parseStoryMaxUploadBytes(value),
                /IMS_STORY_MAX_UPLOAD_BYTES must be/,
            );
        }
    });
    ...
    test("Fudaba geocoding requires an identifiable configurable HTTPS provider", () => {
        assert.deepEqual(parseFudabaGeocodingConfig({}), {
            enabled: false,
            endpoint: "",
            userAgent: "",
            countryCodes: "cn",
        ...
        assert.deepEqual(
            parseFudabaGeocodingConfig({
                IMS_FUDABA_GEOCODING_ENDPOINT:
                    " https://nominatim.example.test/search ",
                IMS_FUDABA_GEOCODING_USER_AGENT:
                    "IMSWeb (contact: op@example.test)",
                IMS_FUDABA_GEOCODING_COUNTRY_CODES: " CN,HK ",
            }),
            {
                enabled: true,
                endpoint: "https://nominatim.example.test/search",
                userAgent: "IMSWeb (contact: op@example.test)",
                countryCodes: "cn,hk",
            },
        );
        assert.throws(
            () =>
                parseFudabaGeocodingConfig({
                    IMS_FUDABA_GEOCODING_USER_AGENT: "IMSWeb test agent",
                }),
            /IMS_FUDABA_GEOCODING_ENDPOINT is required/,
        );
        assert.throws(
            () =>
                parseFudabaGeocodingConfig({
        ...
        assert.throws(
            () =>
                parseFudabaGeocodingConfig({
                    IMS_FUDABA_GEOCODING_ENDPOINT:
                        "https://nominatim.example.test/search",
                    IMS_FUDABA_GEOCODING_USER_AGENT: "short",
                }),
            /10-200 printable characters/,
        );
        assert.throws(
            () =>
                parseFudabaGeocodingConfig({
                    IMS_FUDABA_GEOCODING_ENDPOINT:
                        "https://nominatim.example.test/search",
                    IMS_FUDABA_GEOCODING_USER_AGENT: "IMSWeb test agent",
                    IMS_FUDABA_GEOCODING_COUNTRY_CODES: "china",
                }),
    ...
    test("Fudaba public reads require S3 public object URL configuration", () => {
        assert.doesNotThrow(() =>
            validateFudabaPublicReadStorage(false, {
                type: "s3",
                bucket: "imsweb-media",
                region: "us-east-1",
    ...
    test("Node database accepts only a validated PostgreSQL configuration", () => {
        ...
        assert.deepEqual(
            ...
            {
                connectionString:
                    "postgresql://ims:secret@db.example.test:5432/ims",
                maxConnections: 8,
    ...
    test("FilesystemObjectStorage maps canonical business keys to owned roots", async () => {
        ...
        await assert.rejects(
            storage.publish("wiki/agencies/sc/missing.webp"),
            /Object not found/,
        );
        ...
        assert.deepEqual(
            (await storage.list("wiki/agencies/sc")).map((entry) => entry.key),
            [key],
        );
        await assert.rejects(fs.lstat(path.join(publicDir, key)), {
            code: "ENOENT",
        });
        assert.deepEqual(
            new Uint8Array(
                await fs.readFile(
                    path.join(
                        storyDataDir,
                        "agencies/sc/idols/idol/story-images/card.webp",
                    ),
                ),
            ),
            body,
        ...
        await assert.rejects(fs.lstat(path.join(publicDir, thumbnailKey)), {
            code: "ENOENT",
        });
        await assert.rejects(
            storage.put("Data/sc/idol/card.webp", body),
            /Unsupported object key namespace/,
        );
    });
    ...
    test("NodeStaticAssets does not open a body for HEAD and opens only the requested byte range", async () => {
        ...
        const head = await assets.fetch(
            new Request("http://ims.test/runninggame/Build/game.data", {
                method: "HEAD",
        ...
        const byEtag = await assets.fetch(
            new Request("http://ims.test/runninggame/Build/game.data", {
        ...
        const byDate = await assets.fetch(
            new Request("http://ims.test/runninggame/Build/game.data", {
        ...
        const staleIfRange = await assets.fetch(
            new Request("http://ims.test/runninggame/Build/game.data", {
        ...
        const matchingIfRange = await assets.fetch(
            new Request("http://ims.test/runninggame/Build/game.data", {
        ...
        const invalid = await assets.fetch(
            new Request("http://ims.test/runninggame/Build/game.data", {
    ...
    test("NodeStaticAssets negotiates precompressed assets without exposing encoded files", async () => {
        ...
        const brotliResponse = await assets.fetch(
            new Request("http://ims.test/assets/app-abcdef12.js", {
        ...
        assert.equal(
            brotliResponse.headers.get("content-length"),
            String(brotli.byteLength),
        );
        ...
        const gzipResponse = await assets.fetch(
            new Request("http://ims.test/assets/app-abcdef12.js", {
        ...
        assert.deepEqual(
            gunzipSync(Buffer.from(await gzipResponse.arrayBuffer())),
            source,
        );
        ...
        const range = await assets.fetch(
            new Request("http://ims.test/assets/app-abcdef12.js", {
                headers: {
                    "Accept-Encoding": "br",
                    Range: "bytes=0-4",
        ...
        const unhashed = await assets.fetch(
            new Request("http://ims.test/assets/runtime.js"),
        );
    ...
    test("filesystem idempotency persists replay, rejects fingerprint reuse, and recovers failure", async () => {
        ...
        assert.deepEqual(await first.claim("scope", "key", "fingerprint"), {
            kind: "in-progress",
        });
        await first.complete("scope", "key", "fingerprint", firstClaim.generation, {
            status: 201,
            body: { ok: true },
        });
        ...
        assert.deepEqual(await restarted.claim("scope", "key", "different"), {
            kind: "conflict",
        });
    ...
    test("filesystem compensation journal retries a failed idempotent delete to completion", async () => {
        ...
        const id = await service.enqueue("delete-object", {
            key: "uploads/event/original/a.png",
        });
