...
describe("CommunityPostDetail", () => {
  ...
  it("places structured event data in a dedicated side panel", () => {
    ...
    expect(
      screen.getByRole("link", { name: "活动报名" })
    ).toHaveAttribute("href", "https://example.test/register")
  ...
  it("keeps detail and body images at their natural proportions", () => {
    renderDetail({
      ...baseArticle,
      body_html: '<img src="https://media.example.test/body.webp" alt="正文图片" />',
    })

    expect(screen.getByRole("img", { name: "正文图片" }).parentElement).toHaveClass(
      "[&_img]:object-contain"
    )
    expect(
      screen.getByRole("button", { name: "查看湖南偶像大师 ONLY封面" })
