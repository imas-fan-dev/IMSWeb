...
function renderPage() {
  return render(<MemoryRouter><AdminEventsPage /></MemoryRouter>)
}
...
describe("AdminEventsPage", () => {
  ...
  it("uses the unified community-post APIs and links to the full-page editor", async () => {
    vi.stubGlobal("fetch", vi.fn<typeof fetch>().mockImplementation((input) => {
      ...
      return Promise.resolve(jsonResponse(url.includes("/spotlight") ? { items: [] } : posts))
    }))
    ...
    expect(screen.getByRole("link", { name: "编辑" })).toHaveAttribute("href", "/admin/events/35")
    expect(screen.getByRole("link", { name: "新建文章" })).toHaveAttribute("href", "/admin/events/new")
  })
  ...
  it("manages manually selected homepage spotlight entries", async () => {
    const fetchMock = vi.fn<typeof fetch>().mockImplementation((input, init) => {
      ...
      if (method === "PUT") return Promise.resolve(jsonResponse({ success: true }))
      return Promise.resolve(jsonResponse(url.pathname.endsWith("/spotlight") ? { items: [] } : posts))
    })
    ...
    expect((screen.getByRole("option", { name: "同人活动" }) as HTMLOptionElement).selected).toBe(true)
