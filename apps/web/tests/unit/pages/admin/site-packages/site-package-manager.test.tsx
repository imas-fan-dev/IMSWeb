...
import { SitePackageManager } from "~/pages/admin/site-packages/index"
...
function packagePayload() {
  return {
    ...
    contentOrigin: "http://localhost:5173",
    revisions: [
...
describe("SitePackageManager", () => {
  beforeEach(() => {
    ...
    setCsrfCookie("backoffice", "site-package-test")
    vi.stubGlobal(
      "confirm",
      vi.fn(() => true)
    )
  ...
  it("rotates a preview token, isolates the iframe, and publishes a revision", async () => {
      ...
      if (request.url.endsWith("/preview-token")) {
        return Promise.resolve(
          jsonResponse({
            ...
            previewToken,
            previewUrl: `http://localhost:5173/site-content/_preview/${previewToken}/`,
          })
        )
      }
    ...
    expect(frame).toHaveAttribute(
      "src",
      `http://localhost:5173/site-content/_preview/${previewToken}/`
    )
  ...
  it("confirms and deletes only a historical revision", async () => {
    let currentPackage = packagePayload()
    const fetchMock = installFetchMock((...args) => {
      const request = requestDetails(args)
      if (request.method === "GET") {
      ...
      if (
        ...
        request.url.endsWith(`/${packageId}/revisions/${revisionTwoId}`)
      ) {
        currentPackage = {
          ...currentPackage,
          revisions: currentPackage.revisions.filter(
            (item) => item.id !== revisionTwoId
          ),
        }
        return Promise.resolve(
          jsonResponse({
    ...
    await user.click(screen.getByRole("button", { name: "删除历史版本 2" }))
    const dialog = screen.getByRole("alertdialog")
    expect(dialog).toHaveTextContent("删除历史页面包版本")
    expect(dialog).toHaveTextContent("无法继续预览或回滚")
    await user.click(within(dialog).getByRole("button", { name: "取消" }))
    expect(callsFor(fetchMock, `/revisions/${revisionTwoId}`)).toHaveLength(0)
    ...
    const [request] = callsFor(fetchMock, `/revisions/${revisionTwoId}`)
    expect(request?.method).toBe("DELETE")
    expect(request?.headers.get("X-CSRFToken")).toBe("site-package-test")
  })

  it("keeps a historical revision when the delete request is rejected", async () => {
    installFetchMock((...args) => {
      const request = requestDetails(args)
      if (request.method === "GET") {
        return Promise.resolve(jsonResponse({ packages: [packagePayload()] }))
      }
      if (request.method === "DELETE") {
        return Promise.resolve(
          jsonResponse({ error: "当前线上版本不能删除，请先发布其他版本" }, 409)
        )
      }
      return Promise.reject(new Error(`Unexpected request: ${request.url}`))
    })
    ...
    render(<SitePackageManager />)
    await user.click(
      await screen.findByRole("button", { name: "删除历史版本 2" })
    )
    await user.click(
      within(screen.getByRole("alertdialog")).getByRole("button", {
        name: "删除历史版本",
    ...
    expect(alert).toHaveTextContent("当前线上版本不能删除")
  ...
  it("uploads a new package as multipart and exposes an indeterminate progress state", async () => {
    ...
    expect(screen.getByText("hiro2027.zip")).toBeVisible()
    expect(screen.getByText("ZIP 归档 · 3 B")).toBeVisible()
  ...
  it("accepts a dropped archive and lets the operator remove it", async () => {
    installFetchMock().mockResolvedValue(jsonResponse({ packages: [] }))
    const user = userEvent.setup()

    render(<SitePackageManager />)
    await screen.findByText("0 个页面包")

    const dropZone = screen.getByLabelText("页面包文件选择")
    const archive = new File(["archive"], "dropped-package.zip", {
      type: "application/zip",
    })
    fireEvent.dragEnter(dropZone, {
      dataTransfer: { files: [archive], types: ["Files"] },
    })
    expect(screen.getByText("松开以选择页面包")).toBeVisible()
    fireEvent.drop(dropZone, {
      dataTransfer: { files: [archive], types: ["Files"] },
    })

    expect(screen.getByText("dropped-package.zip")).toBeVisible()
    expect(screen.getByText("ZIP 归档 · 7 B")).toBeVisible()

    await user.click(
      screen.getByRole("button", { name: "移除 dropped-package.zip" })
    )
    expect(screen.getByText("选择页面包归档")).toBeVisible()
    expect(screen.queryByText("dropped-package.zip")).not.toBeInTheDocument()
  })
  ...
  it("blocks an oversized archive and reports a server validation failure", async () => {
    const fetchMock = installFetchMock((...args) => {
      ...
      return Promise.resolve(
        jsonResponse({ error: "页面包包含禁止的活动内容" }, 400)
      )
    })
    const user = userEvent.setup()
    ...
    expect(screen.getByText("文件过大")).toBeVisible()
    expect(archiveInput).toHaveAttribute("aria-invalid", "true")
    await user.click(screen.getByRole("button", { name: "创建并上传" }))

    expect(await screen.findByText("页面包不能超过 80 MiB")).toBeVisible()
    expect(
  ...
  it("renders loading, request failure, and empty states", async () => {
    ...
    installFetchMock()
      ...
      .mockResolvedValueOnce(jsonResponse({ error: "数据库暂不可用" }, 503))
