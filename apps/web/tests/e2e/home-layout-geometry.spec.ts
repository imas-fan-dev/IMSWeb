...
test.describe("home layout geometry", () => {
  test(
  ...
  test("desktop navigation lens stays within its glass segment", async ({
            ...
            try {
              return await activeLink.evaluate((link) => {
                ...
                return (
                  ...
                  Math.abs(
                    Number.parseFloat(lensElement.style.width) -
                      link.offsetWidth
                  ) <= 1
                )
              })
    ...
    async function waitForMeasuredGeometry(activeLink: typeof links) {
      ...
      await expect
        .poll(
          ...
          {
            message:
              "desktop navigation lens should settle over its active link",
            timeout: 15_000,
          }
        )
        .toBe(true)
    ...
    for (let index = 0; index < navigationPaths.length; index += 1) {
      ...
      const activeLink = navigation.locator(
        `a[href="${navigationPaths[index]}"]`
      )
    ...
    const motionSamples = await skin.evaluate((element, ringWidth) => {
      ...
      return [0, 0.28, 0.64, 1].map((progress) => {
        ...
        return {
          ...
          bottomGap:
            segmentRect.bottom - (skinRect.bottom + verticalRingOutset),
        }
      })
    }, ringOutset)
