...
async function settledDialPosition(page: Page, dial: Locator) {
  let previous = await dial.getAttribute("data-wiki-agency-dial-position")
  for (let frame = 0; frame < 4; frame += 1) {
    await page.clock.runFor(16)
    const current = await dial.getAttribute("data-wiki-agency-dial-position")
    if (current === previous) return Number(current)
    previous = current
  }
  throw new Error("Wiki agency dial position did not settle within four frames")
}

test.describe("wiki mobile", () => {
  test(
    "mobile Wiki agency switching preserves both scroll positions",
    {
      tag: "@mobile",
    },
    async ({ page, api, isMobile }) => {
      ...
      const firstIdolCard = page
        .locator('a[aria-label][href^="/story?"]')
        .first()
  ...
  test(
    "modern Wiki windowed dial loops and switches agencies",
    {
      tag: "@mobile",
    },
    async ({ page, api, isMobile }) => {
      ...
      const directionIndicator = dialog.locator(
      ...
      const directionStyles = await directionIndicator.evaluate((element) => {
        const arc = getComputedStyle(
          element.querySelector<SVGPathElement>(
            ".wiki-agency-dial-direction-arc"
          )!
        )
        const head = getComputedStyle(
          element.querySelector<SVGPathElement>(
            ".wiki-agency-dial-direction-head"
          )!
        )
        return {
      ...
      await expect
        .poll(
          async () => (await dial.boundingBox())?.x ?? Number.POSITIVE_INFINITY
        )
      ...
      expect(popupMotion.itemAnimationName).toContain(
        "wiki-agency-dial-item-in"
      )
      expect(
        Math.abs(
          dialBox!.x +
      ...
      const initialContinuousPosition = await settledDialPosition(page, dial)
      expect(initialContinuousPosition).toBeGreaterThan(0.04)
      await expect(outgoingEdgeOption).toHaveCount(1)
      ...
      const firstContinuousPosition = await settledDialPosition(page, dial)
      expect(firstContinuousPosition).toBeGreaterThan(
        initialContinuousPosition + 0.07
      )
      ...
      await settledDialPosition(page, dial)
      await expect(outgoingEdgeOption).toHaveCount(0)
      await page.mouse.move(optionCenterX - 58, optionCenterY, { steps: 6 })
      await expect(dial).toHaveAttribute("data-wiki-agency-dial-dragging")
      const secondContinuousPosition = await settledDialPosition(page, dial)
      ...
      const clockwisePosition = await settledDialPosition(page, dial)
      expect(clockwisePosition).toBeLessThan(beforeTurnPosition - 0.5)
      ...
      const flingStartY =
        continuousDialBox!.y + continuousDialBox!.height * 0.58
      ...
      await dialog
        .getByRole("button", { name: `切换到${previewAgency}` })
        .click()
      await expect
        .poll(() => new URL(page.url()).searchParams.get("agency"))
        .toBe(previewAgency)
      ...
      const directAgency = (await directOption.getAttribute(
        "aria-label"
      ))!.replace("预览企划 ", "")
  ...
  test(
    "classic Wiki follows the mobile content order without narrow title wraps",
    {
      tag: "@mobile",
    },
    async ({ page, api, isMobile }) => {
      ...
      await expect(
        page.getByRole("button", { name: "切换壁纸" })
      ).not.toBeVisible()
      await expect(page.getByRole("button", { name: "返回顶部" })).toHaveCount(
        0
      )
      ...
      await agencyDialDialog
        .getByRole("button", { name: "关闭企划拨盘" })
        .click()
      ...
      await expect(
        page.getByRole("button", { name: "返回顶部" })
      ).not.toBeVisible()
  ...
  test(
    "modern Wiki keeps group navigation and mobile search fixed",
    {
      tag: "@mobile",
    },
    async ({ page, api, isMobile }) => {
      ...
      const searchPanel = page.locator(
        '[data-wiki-mobile-search-panel="modern"]'
      )
      await expect(searchDialog).toBeVisible()
      await expect(searchDialog).toHaveAttribute(
        "data-wiki-mobile-search-dialog",
  ...
  test(
    "classic story portrait cards use two readable mobile columns",
    {
      tag: "@mobile",
    },
    async ({ page, api, isMobile }) => {
  ...
  test(
    "story source labels stay visible in both mobile views",
    {
      tag: "@mobile",
    },
    async ({ page, api, isMobile }) => {
      ...
      for (const route of [
        ...
        const sourcedCard = page
          .locator('[data-story-state="available"]')
          .first()
        ...
        await expect(
          dialog.getByText("Bilibili", { exact: true })
        ).toBeVisible()
  ...
  test(
    "modern story navigation stays clickable over the mobile footer",
    {
      tag: "@mobile",
    },
    async ({ page, api, isMobile }) => {
  ...
  test("new story cards without story sources render in gray", async ({
    ...
    await expect(textOnlyCard).toHaveAttribute(
      "data-story-state",
      "unavailable"
    )
