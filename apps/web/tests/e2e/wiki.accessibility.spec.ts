...
test.describe("wiki accessibility", () => {
  test(
    "wiki archive has no automatically detectable WCAG A/AA violations",
    {
      tag: "@firefox",
    },
    async ({ page, api }) => {
      ...
      await expect(
        page.getByRole("heading", { name: "剧情档案" })
      ).toBeVisible()
      await expect(page.locator('[aria-label="正在加载内容目录"]')).toHaveCount(
        0
      )
