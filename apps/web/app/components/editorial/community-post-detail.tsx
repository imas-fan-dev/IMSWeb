...
export function CommunityPostDetail({
  ...
  return (
    <article className="mx-auto w-full max-w-6xl lg:grid lg:grid-cols-[minmax(0,1fr)_18rem] lg:items-start lg:gap-x-10">
          ...
          <div className={showBackLink ? "mt-7" : ""}>
            ...
            {publisher || publishedAt ? (
              <p className="mt-4 flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground">
                ...
                {publishedAt ? <span>发布于 {formatDateTime(publishedAt)}</span> : null}
              </p>
            ) : null}
            {summary ? (
      ...
      {hasAside ? (
        <aside className="mt-8 rounded-xl border bg-card/85 p-5 shadow-sm lg:col-start-2 lg:row-start-2 lg:mt-9 lg:sticky lg:top-24">
          {hasEventFacts ? (
            <section aria-labelledby="event-details-heading">
              <h2 id="event-details-heading" className="text-base font-semibold">
                活动信息
              </h2>
              <dl className="mt-5 space-y-5">
                ...
                {startAt || endAt ? (
                  ...
                  >
                    ...
                    {endAt ? `${startAt ? "至 " : ""}${formatDateTime(endAt)}` : null}
                  </EventFact>
                ) : null}
                {venueName || address ? (
                  <EventFact
                ...
                {contact ? (
                  <EventFact
                    icon={<ContactRoundIcon aria-hidden="true" className="size-4" />}
                    label="联系方式"
                  >
          ...
          {links.length ? (
            ...
            >
              <h2 id="article-links-heading" className="text-base font-semibold">
                相关链接
              </h2>
