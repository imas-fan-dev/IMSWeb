 ...
 * app) and costs a mouse pointer nothing; pointer layouts keep an explicit
 * refresh control instead.
 *
 ...
 * `transform` never shifts `offsetTop`, so a window virtualizer's
 * `scrollMargin` stays valid while the band is open.
 *
 * Only usable where the window is the scroller. The exchange map is a
 * full-height pane (see `isNonScrollingAppRoute` in `app-shell-scroll.ts`), so
 * `window.scrollY` never leaves zero there and the gesture cannot tell a pull
 * from an ordinary drag.
 *
 * `components/shared/pull-to-refresh.tsx` pairs this with the visible band and
 * is what pages normally use.
 */
