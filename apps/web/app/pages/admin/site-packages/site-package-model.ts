...
export const maximumArchiveBytes = 80 * 1024 * 1024
...
export function warningLabel(warning: string) {
  ...
  if (warning.startsWith("site-icon-ignored:")) {
    return `浏览器图标声明未指向页面包内的图片：${warning.slice("site-icon-ignored:".length)}`
  }
...
export function validateUpload(draft: UploadDraft) {
  const archiveError = validateArchive(draft.archive)
...
export function validateArchive(archive: File | null) {
  if (!archive) return "请选择 ZIP 页面包"
  if (!archive.name.toLowerCase().endsWith(".zip")) {
    return "页面包必须是 .zip 文件"
  }
  if (archive.size > maximumArchiveBytes) {
    return "页面包不能超过 80 MiB"
  }
  return null
}
