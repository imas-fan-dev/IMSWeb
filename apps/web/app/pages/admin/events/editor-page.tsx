...
function formatPublishedAt(value: unknown) {
  ...
  return Number.isNaN(date.valueOf())
    ? source
    : publishedAtFormatter.format(date)
}
...
function initialRelatedLinks(
  ...
  return legacy.filter(
    (link, index) => legacy.findIndex((item) => item.url === link.url) === index
  )
}
...
function SettingLabel({ children }: { children: React.ReactNode }) {
  return (
    <label className="block text-sm font-medium text-foreground">
      <span className="mb-1.5 block text-[13px]">{children}</span>
    </label>
  )
}
...
function CoverCropDialog({
  ...
  const cropAspect =
    ratio === "aspect-video" ? 16 / 9 : ratio === "aspect-5/4" ? 5 / 4 : 4 / 3
  const frameScale = 4 - transform.zoom
  const maxFrameWidth =
    cropAspect >= imageAspect ? 100 : (cropAspect / imageAspect) * 100
  const maxFrameHeight =
    cropAspect >= imageAspect ? (imageAspect / cropAspect) * 100 : 100
  ...
  const frameFocalX = Math.max(
    frameInsetX,
    Math.min(1 - frameInsetX, transform.focalX)
  )
  const frameFocalY = Math.max(
    frameInsetY,
    Math.min(1 - frameInsetY, transform.focalY)
  )
  ...
  function dragFrame(event: React.PointerEvent<HTMLDivElement>) {
    ...
    onChange({
      ...transform,
      focalX: clampFocal(
        drag.focalX + (event.clientX - drag.startX) / drag.width,
        frameInsetX
      ),
      focalY: clampFocal(
        drag.focalY + (event.clientY - drag.startY) / drag.height,
        frameInsetY
      ),
    })
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            详情页始终完整显示原图；这里的焦点与缩放只作用于所有列表缩略图。
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-5">
          ...
          <p className="text-xs text-muted-foreground">
            拖动构图框选择原图区域；图片本身不会被拖动。不同列表比例会共用这套构图。
          </p>
          <label className="block text-sm font-medium">
            取景框大小：{Math.round((frameScale / 3) * 100)}%
            <input
              ...
              onChange={(event) => {
                ...
                const nextWidth = maxFrameWidth * (nextScale / 3)
                const nextHeight = maxFrameHeight * (nextScale / 3)
                onChange({
                  ...transform,
                  focalX: clampFocal(transform.focalX, nextWidth / 200),
                  focalY: clampFocal(transform.focalY, nextHeight / 200),
                  zoom: 4 - nextScale,
                })
              }}
            />
          ...
          <div className="flex justify-between gap-3">
            <Button
              ...
              variant="ghost"
              size="sm"
              onClick={() => onChange(defaultEditorialCoverTransform)}
            >
...
export default function AdminEventEditorPage({ params }: Route.ComponentProps) {
  ...
  const [coverTransform, setCoverTransform] = useState<EditorialCoverTransform>(
  ...
  useEffect(() => {
    if (isNew) return
    let active = true
    void getAdminCommunityPost(Number(params.eventId))
      .send()
      .then((value) => {
        if (!active) return
        setArticle(value)
        ...
        setFields({
          ...
          eventStatus:
            (value.event_status as EventFields["eventStatus"] | undefined) ??
            "scheduled",
        })
        setDirty(false)
      })
  ...
  async function persist(): Promise<EditorialArticle | null> {
    ...
    const result = await updateAdminCommunityPost(
      Number(current.id),
  ...
  async function save(status?: "publish" | "unpublish") {
    ...
    try {
      ...
      if (!current) return
      if (status) {
        const result = await setAdminCommunityPostStatus(
          Number(current.id),
          status,
          current.revision
        ).send()
        setArticle({
          ...current,
          status: result.status as EditorialArticle["status"],
          revision: result.revision,
        })
      }
      toast.success(
  ...
  const statusOptions: Array<[EventFields["eventStatus"], string]> = [
    ["scheduled", "已排期"],
    ["ongoing", "进行中"],
    ["ended", "已结束"],
    ["cancelled", "已取消"],
  ...
  if (loadError)
    return (
      <AdminEmptyState
        icon={FileTextIcon}
        title="无法打开文章"
        description="请返回文章工作台后重试。"
      />
    )

  return (
    <div className="-mx-4 -my-7 min-h-[calc(100svh-4rem)] bg-muted/15 sm:-mx-6 lg:-mx-8 lg:-my-9 xl:-mx-10">
      <header className="sticky top-16 z-30 flex min-h-16 items-center justify-between gap-3 border-b bg-background/95 px-4 py-3 backdrop-blur sm:px-6 lg:px-8 xl:px-10">
        <div className="flex min-w-0 items-center gap-3">
          <NavigationLink href="/admin/events">
            <Button variant="ghost" size="icon-sm" aria-label="返回文章工作台">
              <ArrowLeftIcon />
            </Button>
          </NavigationLink>
          <div className="hidden min-w-0 sm:block">
            ...
            <p className="mt-0.5 text-xs text-muted-foreground">
              {dirty
                ? "有未保存的更改"
                : isPublished
                  ? `已发布 · ${formatPublishedAt(article?.published_at)}`
                  : "草稿 · 所有更改已保存"}
            </p>
          </div>
        </div>
        ...
        <div className="grid items-start gap-x-10 lg:grid-cols-[minmax(0,1fr)_18rem]">
          <header className="min-w-0 lg:col-start-1">
            <div className="flex flex-wrap items-center gap-3">
              <select
                aria-label="文章类型"
                className={`${adminControlClass} w-auto`}
                value={kind}
                onChange={(event) => {
              ...
              >
                <option value="notice">社区动态</option>
                <option value="event">具体活动</option>
              </select>
              <span className="text-xs text-muted-foreground">
                {isNew ? "新建文章" : isPublished ? "已发布文章" : "草稿编辑中"}
              </span>
            </div>
            <div className="mt-6">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                >
                  <span className="grid size-8 place-items-center rounded-md border bg-primary/10 text-primary">
              ...
              <Input
                ...
                placeholder="为这篇文章写下一个清晰的标题"
                className="mt-3 h-auto rounded-none border-x-0 border-t-0 border-b-2 bg-transparent px-0 py-3 text-4xl font-semibold tracking-tight shadow-none placeholder:text-muted-foreground/45 focus-visible:border-primary focus-visible:ring-0 sm:text-5xl"
              />
            </div>
            <div className="mt-7 rounded-xl border bg-background/70 p-4 shadow-xs sm:p-5">
              ...
              <Textarea
                ...
                placeholder="用一两句话概述文章亮点，让读者愿意继续阅读。"
                className="mt-4 min-h-28 resize-y border-0 bg-transparent p-0 text-lg/8 shadow-none placeholder:text-muted-foreground/55 focus-visible:ring-0 sm:text-xl/9"
              />
            </div>
          </header>
          ...
          <main className="mt-9 min-w-0 rounded-xl border bg-card/85 p-4 shadow-sm sm:p-6 lg:col-start-1 lg:row-start-2">
            <section aria-label="文章封面">
              <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
                <h2 className="text-lg font-semibold">文章封面</h2>
                <p className="text-xs text-muted-foreground">
                  详情页完整显示，列表按构图裁切。
                </p>
              </div>
              <div className="overflow-hidden rounded-lg border bg-muted">
                {coverUrl ? (
                  ...
                  <div className="grid min-h-52 place-items-center gap-2 px-5 text-center text-sm text-muted-foreground">
                    <ImageUpIcon className="size-6" />
                    <span>给文章添加一张封面图</span>
                  </div>
                )}
              </div>
            ...
            <section className="mt-9 border-t pt-8" aria-label="文章正文">
              ...
              <RichTextEditor
                ...
                onUpload={(file) => upload(file, "body")}
              />
            </section>
            <p className="mt-6 border-t pt-4 text-xs text-muted-foreground">
              {bodyReady
                ? "正文已有内容，可继续完善并保存。"
                : "从这里开始写作，完成后可在顶部保存或发布。"}
            </p>
          </main>
          ...
          <aside className="mt-8 min-w-0 rounded-xl border bg-card/85 p-5 shadow-sm lg:sticky lg:top-24 lg:col-start-2 lg:row-start-2 lg:mt-9">
                ...
                <div className="mt-5 space-y-4">
                  <SettingLabel>
                    ...
                    <select
                      ...
                      value={fields.eventStatus}
                      onChange={(event) => {
                        setFields((current) => ({
                          ...current,
                          eventStatus: event.target
                            .value as EventFields["eventStatus"],
                        }))
                        markDirty()
                      }}
                    >
            ...
            <section
              className={kind === "event" ? "mt-6 border-t pt-5" : ""}
              aria-labelledby="links-settings-heading"
            >
              <div className="flex items-center justify-between gap-3">
                <h2
                  id="links-settings-heading"
                  className="text-base font-semibold"
                >
                  相关链接
                </h2>
                <span className="text-xs text-muted-foreground">
                  {relatedLinks.length}/20
                </span>
              </div>
              <p className="mt-1 text-xs/5 text-muted-foreground">
                站内链接以 / 开头；外部链接仅支持 HTTP(S)。
              </p>
                  ...
                  >
                    <div className="flex items-center gap-1">
                      ...
                      <Button
                        ...
                        variant="ghost"
                        aria-label="下移链接"
                        disabled={index === relatedLinks.length - 1}
                        onClick={() => moveRelatedLink(index, 1)}
                      >
                        <ChevronDownIcon />
                      </Button>
              ...
              <Button
                ...
                variant="outline"
                size="sm"
                disabled={relatedLinks.length >= 20}
                onClick={() => {
                  setRelatedLinks((current) => [
                    ...current,
                    { label: "", url: "" },
            ...
            <section className="mt-6 border-t pt-5">
              <h2 className="text-base font-semibold">发布设置</h2>
              <div className="mt-4 space-y-4">
                ...
                <div className="rounded-lg bg-muted/40 p-3 text-xs">
                  <div className="flex items-center gap-2 font-medium">
                    <CheckCircle2Icon
                      className={`size-4 ${isPublished ? "text-success" : "text-muted-foreground"}`}
                    />
                    {isPublished ? "已发布" : "草稿"}
                  </div>
                  <p className="mt-1 text-muted-foreground">
                    {isPublished
                      ? `发布于 ${formatPublishedAt(article?.published_at)}`
                      : "保存草稿不会公开文章。"}
                  </p>
                </div>
                <div className="space-y-2 text-xs">
                  {[
                    [Boolean(title.trim()), "标题"],
                    [Boolean(coverUrl), "封面"],
                    [bodyReady || Boolean(sourceUrl), "正文或来源"],
                  ].map(([complete, label]) => (
                    <div
                      key={label as string}
