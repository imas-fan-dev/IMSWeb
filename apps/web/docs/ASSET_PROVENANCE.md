...
`public/maps/china-boundary-dashes.json` 于 2026-08-18 从同一个上游接口
`https://geo.datav.aliyun.com/areas_v3/bound/100000_full.json`（响应 `Last-Modified` 为
2021-06-29 11:16:54 GMT）取出其中的 `100000_JD` 要素，几何与 properties 逐字节保留，仅外层
包了一个 `FeatureCollection`。文件大小 7874 字节，SHA-256 为
`f30afc651f83a8da6e203889dc3fa3de46ae3588be23cde69e979961dca590cd`。该要素是南海断续线：
MultiPolygon、10 段、bbox `108.1968, 3.3972 – 122.7989, 24.5861`，最南端 3.3972°N 位于曾母暗沙
以南。上游把每段断续线存为宽约 0.02 度的细长面（描边转面、圆头端帽）而非线，因此交换地图
用同色的 fill 与 outline 双层渲染：低缩放靠 outline 保证可见，高缩放靠 fill 填实。

加入该文件是为满足《公开地图内容表示规范》（自然资规〔2023〕2 号）第五条对中国全图绘出南海
断续线的要求。**授权状态与上一文件相同且需注意**：阿里云 DataV GeoAtlas 帮助文档写明其数据
「仅供学习和交流使用」，并未授予公开商业分发许可，该文件亦无审图号。因此它不标记为 MIT，
其公开部署与再分发沿用仓库维护者对上一文件已作出的同一份授权范围决定，不自动扩展到其他
用途或再许可。若后续需要法律层面的完整覆盖，须改用经自然资源主管部门审核并取得审图号的
数据，或就本地图整体送审。
