...
App 使用首页、社区、交换地图、资料、我的五个一级入口。顶栏标识所属栏目，子页面在内容区
显示自己的标题；由一级入口改为子页面时，也要恢复可见标题。栏目首页不显示返回按钮，
整屏交换地图保留自身控件与顶栏排除规则。栏目切换恢复阅读位置，重选回栏目首页，再次
重选回顶；交换地图根页保留视角、筛选及 URL，不执行窗口回顶。公开事务所详情归交换地图，
个人工作区仍归我的。具体归属与交互由 App 导航模型和协调层统一管理。
...
条目本身是 `glass-tab`，按下时缩放到 `--glass-tab-press`。网站顶栏与 App 的 Web 回退底栏共用
...
Web 暂不启用指针追踪高光。生产组件不得使用 `data-glass-interactive`、`glass-sheen` 或
`glass-control`，根布局也不挂载 `GlassSheenTracker`，因此 Header、面板、卡片和 Web App 底栏
都不追踪指针位置，也不生成白色触点高光。静态玻璃材质、折射、舌片透镜和普通按压缩放继续使用。
现有 tracker 与 CSS 规则只作为停用实现保留；重新启用前必须完成交互设计和桌面、触屏浏览器验证。
...
Web 指针追踪高光与触点形变当前停用，不属于生产交互。iOS 26 及以上仍使用系统
`UITabBarController` 的默认 Liquid Glass 反馈；其他 Web 与 App 回退只保留静态玻璃、舌片透镜和
普通按压缩放。重新启用触点高光时不得直接恢复挂载，必须先解决嵌套表面归属、拖动边界、焦点残留和
触屏退出状态，并重新验证减少动态效果设置。

`prefers-reduced-motion` 启用时，透镜的位移过渡、形变动画、图标回弹和按下缩放全部取消，切换
变成一次即时状态改变。
