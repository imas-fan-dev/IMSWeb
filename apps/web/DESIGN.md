...
即使包含交互子元素，也不得因事件冒泡产生形变、回弹或触点变量；它们只保留原有静态材质和悬停高光。
Android 与 iOS 26 以下的 App 底栏只保留 Web 玻璃材质、透镜和按压缩放，不得带
`data-glass-interactive` 或 `glass-sheen`，因此不追踪手指位置，也不生成白色触点高光。
...
Web 中显式标记的交互玻璃，其触点反馈分四段：按下后在 `--duration-fast` 内从触点扩散高光并
压缩表面；持续按住时，触点成为 `transform-origin`；拖动按每帧更新，位移限制在横向 6px、纵向
4px，并按主要方向做有限拉伸；释放后在 `--duration-ui` 内经过一次轻微越界和反向压缩回到静止。
手指或指针一旦越出交互表面的实际边界，立即取消按压与回弹；最后一帧高光固定在移动轨迹与边界
的真实交点，再在 `--duration-fast` 内淡出。移动端 App 底栏不使用这套位置追踪；iOS 26 及以上交给系统 `UITabBarController` 的默认交互和
Liquid Glass 反馈，其他移动端回退只保留无白色高光的按压缩放。Web 移动端不启用
CSS hover，避免抬手或链接获得焦点后残留高光。文字与图标只跟随受力位移，不改变布局尺寸。触点
光效只动画 `opacity` 和 `transform`，不动态修改模糊或折射滤镜。

`prefers-reduced-motion` 启用时，透镜的位移过渡、形变动画、图标回弹、按下缩放、触点扩散和拖动
跟随全部取消，切换变成一次即时状态改变。
