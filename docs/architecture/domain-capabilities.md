...
> 文档类型：架构
> 状态：Active
> 权威来源：`apps/api/src/domains/README.md`、`apps/api/scripts/checks/hono-architecture.js` 和各 domain `routes.ts`
...
本设计约束当前目录结构、组合边界和后续维护顺序，不允许为了目录整理改变公开 URL、HTTP
method、认证策略、响应字段、数据库 schema 或对象键。目录变更必须按能力逐步完成，并以
现有契约测试作为回归依据。
...
API domain 固定归入 `identity`、`admin`、`content`、`community` 和 `delivery` 五个产品板块。
Domain、模块和 handler 清单由 `apps/api/scripts/checks/hono-architecture.js` 与
`apps/api/src/domains/README.md` 维护，不在文档中复制易漂移的数量快照。`reactions` 属于
`namecards` capability，不是独立 domain。
...
domain（如 `chronicle`、`information`）保持扁平，不建能力目录；S 级 domain（如
`site-packages`）同样不应因为拥有
...
Web endpoint 目录与 contracts 的业务布局保持同构：单文件业务保持平铺，只有多文件业务
提升为文件夹。当前入口如下：
...
这些 endpoint 模块只负责请求输入、File/FormData 形状、Alova 方法和 Web-only 类型；响应
schema、wire types 和路径前缀分别来自 `@imsweb/contracts` 的业务子路径、`@imsweb/contracts/z`
和 `@imsweb/contracts/paths`。页面按用户流程分组，根页面只做编排；例如
`community/exchange` 可将公开目录、办公室、卡片和地图分别放入子目录。

## 结构变更流程

1. 新增或移动 domain/capability 前，先更新 `apps/api/src/domains/README.md` 和对应 domain
   README，明确 action、权限、URL、request parser、response、port 和测试。
2. 保持根 `routes.ts` 只做 capability 组合；挂载式 capability 使用
   `createCapabilityRouter()`，历史异构 URL 使用 registrar，不能在根文件重新绑定 handler。
3. 改动跨端响应时，先修改 `packages/contracts/src/<domain>.ts`、package exports 和根 namespace，
   再同步 API response alias、Web endpoint import 与 HTTP conformance assertion。
4. 改动 URL 前缀时只修改 `packages/contracts/src/paths.ts`；调用方只保留业务 suffix 和动态参数。
5. 不在同一个变更中同时重命名路由、改变数据库 port 和搬迁目录；每一步保持 method、middleware
   顺序、响应字段和对象 URL 行为不变。
6. 旧文件只有在 importer 为零、架构检查和 route contract 都确认后才能删除。
