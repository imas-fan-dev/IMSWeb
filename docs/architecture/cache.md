...
> 文档类型：架构
> 状态：Active
> 权威来源：`apps/api/src/ports/cache.ts`、`apps/api/src/infra/cache/` 和 `apps/api/src/runtime/node-services.ts`

## 目标

IMSWeb 的缓存用于短期、可丢失的数据，不承担账户、验证码或审核状态的权威存储。共享运行时使用 Valkey，并保留 Memory 实现供测试和无外部依赖的运行时使用。
...
- idempotency store 和审核 CAS 状态不得迁移到 Valkey；这些能力需要持久化和事务语义，继续使用 PostgreSQL。
