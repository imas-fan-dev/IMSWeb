...
> 文档类型：架构
> 状态：Active
> 权威来源：`apps/api/src/domains/content/wiki/`、`packages/contracts/src/wiki.ts` 和 Wiki contract tests
...
Entry-type migration 在兼容表 `idols` 上新增 `entry_kind` 和 `entry_subtype`。界面把所有记录称为
...
兼容策略只改变业务叫法和增加类型元数据。`idols`、`idolId`、公开查询参数 `idol=` 以及既有对象存储
路径仍是兼容契约，避免破坏书签、外部客户端和已有媒体；新代码不得根据内容页类型改写这些标识。

### 规范化剧情

规范化剧情 migration 新增 `wiki_story_cards` 和 `wiki_story_links`。运行时查询和写入均使用
这两张表，不再按企划选择不同的剧情表：
...
来源目录 migration 把内容类型和来源平台从来源文本中拆成可管理目录。同一张卡片可由
...
API 启动会检查 `ims_schema_migrations` 是否包含代码要求的 migration；缺失时拒绝启动并提示迁移。
已经执行的 migration 不得修改或删除。

### 生产环境

生产迁移是独立发布操作，不由代码合入、本地测试或 `pnpm dev` 代替。执行前必须确认生产数据库
URL、备份、维护窗口和对象存储目标；执行后必须回读：

1. migration ledger 中所有代码要求的版本，且 checksum 无漂移；
2. 历史剧情到规范化卡片/来源的双向投影和计数守恒；
3. 企划、栏目、内容页、页面类型及多栏目归档关系；
4. 非空媒体逻辑键和对象版本；
5. 公开 catalog/stories 与后台工作台的实际 HTTP 行为。

历史表如果仍承担兼容读取，必须保持只读并由对账流程覆盖；新代码不得继续写入历史表。回滚旧代码
前必须确认其 schema、contract 和对象状态机兼容性。生产切换需要独立审批，本地门禁通过不等于
生产切换完成。
...
回读；fixture、空数据库或仅构建成功不能替代生产数据验收。
