...
> 文档类型：运维
> 状态：Active
> 权威来源：`scripts/deployment/`、`deploy/`、`.github/workflows/` 和 release artifact checks
> 适用环境：生产或等价 staging；执行前必须确认目标、备份和回滚负责人
...
使用标准 AWS 凭据链，不写入仓库。完整说明见 [Node 文件对象存储](../architecture/object-storage.md)。
